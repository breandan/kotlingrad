...
boxsep=-1.5pt,
underlay={%
\path[draw=none] (interior.north west) rectangle node[white]{\includegraphics[width=5mm]{laptop.png}} ([xshift=-10mm,yshift=-9mm]interior.north west);
},
    overlay first={
        \node[black!50] (S) at (frame.south) {\LARGE\ding{34}};
        \draw[dashed,black!50] (frame.south west) -- (S) -- (frame.south east);
...
boxsep=-1.5pt,
underlay={%
\path[draw=none] (interior.north west) rectangle node[white]{\includegraphics[width=8mm]{raspi.png}} ([xshift=-10mm,yshift=-8mm]interior.north west);
},
    overlay first={
        \node[black!50] (S) at (frame.south) {\LARGE\ding{34}};
        \draw[dashed,black!50] (frame.south west) -- (S) -- (frame.south east);
...
\begin{equation}
    \begin{split}
        \mathbf{\hat{f}}(\mathbf{x}) = \mathbf{\beta}^\intercal \varphi_{\odot} \left(\mathbf{\theta}^\intercal \mathbf{x}\right + \mathbf{b}) \\
...
\section{Background}
%
\noindent Suppose we have a differentiable vector function $\mathbf{\hat P}: \mathbb{R}^n\rightarrow\mathbb{R}^m$ as seen earlier:
%
...
\end{equation}
%
In white box adversarial learning, we are given a fixed $\mathbf \Theta$ and our goal is to find:
%
\begin{equation}
...
\end{equation}
%
Imagine a single test $T: \mathbb{R}^n \rightarrow \mathbb{B}$. Consider the following example:
%
\begin{equation} \label{eq:output_constraint_example}
    T(\mathbf{\hat P}) = \forall \mathbf{x} \in [0, 1]^m, \mathbf{P}(\mathbf{x}) < \mathbf{C}
\end{equation}
%
...
Another strategy, independent of how candidate inputs are selected, is to use some form of gradient based optimization in the search procedure. The gradient of $\mathbf{\hat P}$'s loss with respect to $\mathbf{x}$ (assuming $\mathbf{\Theta}$ is fixed\footnote{In contrast with backpropogation, where the parameters are updated.}) is $\nabla_{\mathbf{x}} \mathcal{L}\big(\mathbf{\hat P}(\mathbf{x}; \mathbf{\Theta}), \mathbf{y}\big)$. Instead of minimizing the loss, we want to maximize it. So the vanilla gradient update step in~\ref{eq:stochastic_grad_descent} becomes:
%
\begin{equation}
    \mathbf{x} \leftarrow \mathbf{x} + \alpha \frac{1}{z} \nabla_{\mathbf{x}} \mathcal{L}\big(\mathbf{\hat P}(\mathbf{x}; \mathbf{\Theta}), \mathbf{y}\big)
\end{equation}
%
...
We could also sample from outside the feasible set and perform gradient descent over the objectives to perform the equivalent of "shrinking" from property based testing. TODO
...
This procedure is identical to the snapshot method performed in Sec. ~\ref{subsec:creating-an-image}, but the result is much cleaner. Now, instead of maintaining a 869MB BLOB, we can just store the 4KB text file. Similar to before, we can simply run:
...
Now if we put some text in \inline{duck.txt} and rebuild our image, the file will be copied into the Docker image, and its contents will be printed:
...
This ensures the package list is not baked into the image (Docker will only checkpoint the layer after each step is complete). Often, complex builds can take several steps, despite only producing a single artifact. Instead of chaining together several commands and cleaning up the changes in a single step, multi-stage builds let us to build a series of images inside a \inline{Dockerfile}, and copy resources from one to another, discarding all intermediate build artifacts:
...
\begin{enumerate}
    ...
    \item Micro SD card (16GB+ reccommended)
    \item Personal computer
    \item Internet-enabled router
    \item MicroSD card adapter
\end{enumerate}
%
In addition, the following software are assumed:
%
\begin{enumerate}
    \item POSIX-compliant shell
    \item Browser and/or Docker CE
    \item wget/curl
    \item duckietown-shell
    \item Docker
\end{enumerate}

When developing Docker containers, there are two paths to deployment. You can write the Dockerfile on your laptop or an x86 machine, then build with the \inline{RUN [ "cross-build-start" ]} and \inline{RUN [ "cross-build-end" ]} commands. Once tested, you can deploy to the Duckiebot directly by running the following command:

\begin{pclisting}
~# docker save TAG_NAME | ssh -C duckie@DUCKIEBOT_NAME.local docker load
\end{pclisting}

Alternately, you can build directly on an ARM device by creating a file named Dockerfile.arm (the \inline{.arm} extension is just for the reader’s benefit), adding a base image and some build instructions, and running the command:
...
Note that ARM-specific \inline{Dockerfile}s will not build on non-Mac x86 machines, and attempting to build one will cause an error on Docker Hub. However, once you have debugged the \inline{Dockerfile} on an ARM device, you can easily port the entire build to x86 by enclosing it with \inline{RUN [ "cross-build-start" ]} and \inline{RUN [ "cross-build-end" ]} instructions, after the \inline{FROM} and before the \inline{CMD} directive.
...
Here \inline{RUN_COMMAND} may be a shell such as \inline{/bin/bash} or another command such as \inline{/bin/bash -c "roscore"}. The \inline{qemu3-arm-static} entrypoint is provided by the base image, \inline{duckietown/rpi-ros-kinetic-base}.
...
And visit to the following URL: http://DUCKIEBOT_NAME.local:8082/

\subsection{Testing the camera}

Open Portainer Web interface and run the \inline{duckietown/rpi-docker-python-picamera} container.

Publish port \inline{8081} and ensure that the container is run in ``Privileged'' mode.

\subsection{Portainer PiCam Demo}
...
The syntax \inline{-H DUCKIEBOT_NAME.local} may be omitted if running the command over SSH.
...
To allow incoming X connections on Linux hosts, run \inline{xhost +} on your computer.\footnote{There is a more secure way to do this to prevent arbitrary X11 connections.}

The above command opens a ROS shell running on your laptop that is set to connect to DUCKIEBOT‘s ROS Master. To test the ROS connection, run \inline{roswtf}.
