\chapter{Tester des systèmes intelligents}\label{ch:difftest}
...
fun <I, O> gen(program: (I) -> O, property: (O) -> Boolean, rand: () -> I) =
    repeat(1000) {
        ...
        assert(property(program(randomInput))) {
            ...
            "Contre-exemple d'entrée minimal de propriété: $shrunken"
        }
    }
\end{kotlinlisting}
%
...
\If {$m = 1$} \Comment{Évaluer $\mathbf{\hat P}$ et retourner la meilleure perturbation variable}
...
\Else \Comment{Monter le gradient pour une étape en utilisant la différence finie par composant}
...
\While{$0 < \mathcal{B}_e$}\Comment{Itérer jusqu'à épuisement du budget.}
\State $\mathbf{x}_j \sim \mathcal S_m$\Comment{Tirez un échantillon de $\mathcal S_m$}
\If {$\mathbf T\big(\mathbf{x}_j, C\big)$} \Comment{À l'intérieur de l'ensemble réalisable, effectuez une ascension en gradient}
...
\State $i \gets 1, t_i \gets \mathbf T\big(\mathbf x_i, C\big)$ \Comment{Stockez l'état initial pour détecter quand le test bascule.}
\Do
    \State $i \gets i + 1, \mathbf x_i \gets \bm\Phi_{CS}\big(\mathbf x_{i-1} - \alpha\mathbf\nabla_{\mathbf x} \mathcal{L}(\mathbf{x}_{i-1})\big)$ \Comment{PGD étape (\autoref{eq:projected_gd})}
    \If {$\mathbf T\big(\mathbf{x}_i, C\big) \neq t_1$} \Comment{Une valeur limite a été trouvée.}
    \State \Return \textbf{if } $t_1$ \textbf{ then } $\{\mathbf{x}_{i}\}$ \textbf{ else } $\{\mathbf{x}_{i-1}\}$ \textbf{ end if} \Comment{Toujours renvoyer la violation.}
    \EndIf
\doWhile {$i \leq I_{max}$ \textbf{and} $\epsilon < \mathbf |\mathcal{L}(\mathbf x_i) - \mathcal{L}(\mathbf x_{i-1})|$} \Comment{Faire sans converger.}
\State \Return \textbf{if } $\neg t_1$ \textbf{ then } $\{\mathbf x_{i-1}\}$ \textbf{ else } $\varnothing$ \Comment{Retourne la dernière itération ou $\varnothing$.}
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\Do
    \State $\mathbf{x} \sim \mathcal S_m, \mathbf{y} \gets \mathcal{O}(\mathbf{x}), \mathcal{B}_l \gets \mathcal{B}_l - 1$ \Comment{Demandez nouveau label de l'oracle.}
...
\begin{figure}[H]
\begin{tikzpicture}
\begin{axis}[title={Efficacité moyenne au seuil d'erreur $\sigma$ écarts types au-dessus de l'ESM}, width=\textwidth, height=7cm, xlabel=$\sigma$, ylabel=Erreurs moyennes détectées par label, legend pos=north east, align=center, xtick distance=0.5]
...
\end{axis}
\end{tikzpicture}
\caption{Par construction, notre rétrécisseur détecte un plus grand nombre d'erreurs par évaluation que celui qui ne prend pas en compte le gradient.}
\label{fig:pbt_comparison}
\end{figure}
