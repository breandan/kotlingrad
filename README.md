...
|              Math<sup>†</sup>              |             Infix             |              Prefix              |         Postfix<sup>‡</sup>         |                                                    Type                                                    |
|:------------------------------------------:|:-----------------------------:|:--------------------------------:|:-----------------------------------:|:----------------------------------------------------------------------------------------------------------:|
...
| **A** / **B**<sup>§</sup><br>**A** **B**⁻¹ |     `a / b`<br>`a.div(b)`     |           `div(a, b)`            |                                     | (`a`: ℝ<sup>#</sup>→ℝ<sup>M×N</sup>, `b`: ℝ<sup>~</sup>→ℝ<sup>P×N</sup>) → (ℝ<sup>?</sup>→ℝ<sup>M×P</sup>) |
|              -**A**<br>+**A**              |                               |           `-a`<br>`+a`           | `a.unaryMinus()`<br>`a.unaryPlus()` |                                  (`a`: ℝ<sup>#</sup>→ℝ*) → (ℝ<sup>#</sup>→ℝ*)                                   |
|       **A** + **1**<br>**A** - **1**       |    `a + one`<br>`a - one`     |          `++a`<br>`--a`          | `a++`,`a.inc()`<br>`a--`,`a.dec()`  |                     (`a`: ℝ<sup>#</sup>→ℝ<sup>M×M</sup>) → (ℝ<sup>#</sup>→ℝ<sup>M×M</sup>)                      |
|      *sin*(a)<br>*cos*(a)<br>*tan*(a)      |                               | `sin(a)`<br>`cos(a)`<br>`tan(a)` | `a.sin()`<br>`a.cos()`<br>`a.tan()` |                                               (`a`: ℝ→ℝ) → (ℝ→ℝ)                                                |
|                *ln*(**A**)                 |                               |       `ln(a)`<br>`log(a)`        |        `a.ln()`<br>`a.log()`        |                     (`a`: ℝ<sup>#</sup>→ℝ<sup>M×M</sup>) → (ℝ<sup>#</sup>→ℝ<sup>M×M</sup>)                      |
|          *log*<sub>b</sub>(**A**)          |          `a.log(b)`           |           `log(a, b)`            |                                     |        (`a`: ℝ<sup>#</sup>→ℝ<sup>M×M</sup>, `b`: ℝ<sup>~</sup>→ℝ<sup>M×M</sup>) → (ℝ<sup>?</sup>→ℝ)        |
|             **A**<sup>b</sup>              |          `a.pow(b)`           |           `pow(a, b)`            |                                     |        (`a`: ℝ<sup>#</sup>→ℝ<sup>M×M</sup>, `b`: ℝ<sup>~</sup>→ℝ) → (ℝ<sup>?</sup>→ℝ<sup>M×M</sup>)        |
|                  √a<br>∛a                  | `a.pow(1.0/2)`<br>`a.root(3)` |      `sqrt(a)`<br>`cbrt(a)`      |      `a.sqrt()`<br>`a.cbrt()`       |                           (`a`: ℝ<sup>#</sup>→ℝ<sup>M×M</sup>) → (ℝ→ℝ<sup>M×M</sup>)                            |
| <sup>da</sup>&frasl;<sub>db</sub><br>a'(b) |          `a.diff(b)`          |           `grad(a)[b]`           |            `d(a) / d(b)`            |                            (`a`: ℝ<sup>M</sup>→ℝ, `b`: ℝ→ℝ) → (ℝ<sup>M</sup>→ℝ)                            |
|                     ∇a                     |                               |            `grad(a)`             |             `a.grad()`              |                                   (`a`: ℝ<sup>M</sup>→ℝ) → (ℝ<sup>M</sup>→ℝ)                                    |
...
&sect; Matrix division is defined iff **B** is invertible, although it could be possible to redefine this operator using the [Moore-Penrose inverse](https://en.wikipedia.org/wiki/Moore%E2%80%93Penrose_inverse).
