...
\begin{document}
    ...
    \begin{frame}[fragile]
        \frametitle{Abstract syntax tree construction}
    ...
    \begin{frame}[fragile]
        ...
        \begin{lstlisting}[language=Kotlin, gobble=12, style=backtickstyle]
            interface Nat<T: `2`> { val value: Int }
            ...
            open class `2`(open val value: Int = 2)
              { companion object: `2`(), Nat<`2`> }
            // <L: `2`> accepts L <= 2 via Liskov substitution
            class Vec<E, L: `2`>(val len: L, val es: List<E>)
            operator fun <L: `2`, V: Vec<Int, L>> V.plus(v: V) = Vec<Int, L>(len, es.zip(v.es).map{it.l + it.r})
