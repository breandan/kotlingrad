...
listing only,
underlay={%
\path[draw=none] (interior.south west) rectangle node[white]{\includegraphics[width=5mm]{laptop.png}} ([xshift=-10mm]interior.north west);
},
}
...
listing only,
underlay={
\path[draw=none] (interior.south west) rectangle node[white]{\includegraphics[width=8mm]{raspi.png}} ([xshift=-10mm]interior.north west);
},
}
...
It would be nice to give a computer -- any computer with an internet connection -- a short string of ASCII characters, press enter, and return to see some program running. Never mind where the program was built or what software happened to be running at the time. This may seem trivial, but is a monumental software engineering task to implement.

Today, we can run any Linux program on almost any networked device on the planet. All of the environment preparation, installation and configuration steps can be automated from start to finish. Depending on how much network bandwidth is available, it might take some time, but we will rarely need to intervene in the installation process.
