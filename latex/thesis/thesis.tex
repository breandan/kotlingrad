...
For example, suppose that our MR is $\mathbf P(\mathbf x) = \mathbf P(\mathbf x' = \mathbf x + \varphi) = \mathbf y, \forall \varphi \in [0, \varepsilon]$. Given a small set of input-output pairs from an oracle $\langle \mathbf x, \mathbf y \rangle \in \mathbf P$, and a program $\mathbf{\hat P}$, the MR suggests a much larger set of inputs $\{\mathbf x: |\mathbf x - \mathbf x'| < \varepsilon\}$ on which to test $\mathbf{\hat P}$, without requiring corresponding outputs from the oracle $\mathbf P$. If $\exists \mathbf x' \in \{\mathbf x: |\mathbf x - \mathbf x'| < \varepsilon \mid \mathbf P(\mathbf x) \neq \mathbf{\hat P}(\mathbf x')\}$, this implies $\mathcal R$ does not hold on $\mathbf{\hat P}$ and either:

\begin{enumerate}
\item the MR is invalid $(\langle \mathbf x, \mathbf P(\mathbf x)\rangle, \langle \mathbf x', \mathbf P(\mathbf x')\rangle) \notin \mathcal R$, or
\item $\mathbf{\hat P} \neq \mathbf{P}$ the program under test is broken.
\end{enumerate}
