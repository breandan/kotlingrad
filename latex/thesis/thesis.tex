...
This leads us to adversarial testing. Imagine we would like to change the inputs of a known function $\mathbf{\hat P}: \mathbb R^m \rightarrow \mathbb R $ by a small amount, so as to produce the largest change in its output -- forget what we know about derivatives or differentiability for a moment. Suppose that for each input parameter $g_0, g_1, \ldots, g_{n-1}$, we have one of three choices to make: either increase the value by a fixed amount, decrease the value by the same amount, or leave it unchanged. Consider the na\"ive solution, in which we we try every direction:

\begin{algorithm}[H]
\caption{Na\"ive Fuzzer}
\label{alg:naive_fuzz}
\begin{algorithmic}[1]
\Procedure{Na\"iveFuzz}{$\mathbf{\hat P}: \mathbb{R}^n \rightarrow \mathbb{R}, g_0: \mathbb R$, $g_1: \mathbb R$, $\ldots$, $g_{n-1}: \mathbb R$}
\If {$n = 1$}
\State \Return $\argmax{g_0}\{\mathbf{\hat P}(g_0 + c), \mathbf{\hat P}(g_0 - c), \mathbf{\hat P}(g_0)\}$
\Else
\State \Return $\argmax{g_0, \ldots, g_{n-1}}\{\Call{Na\"iveFuzz}{$\mathbf{\hat P}(g_0 + c), g_1, \ldots, g_{n-1}$},\newline
    \hspace*{10.5em} \Call{Na\"iveFuzz}{$\mathbf{\hat P}(g_0 - c), g_1, \ldots, g_{n-1}$},\newline
    \hspace*{10.5em} \Call{Na\"iveFuzz}{$\mathbf{\hat P}(g_0), g_1, \ldots, g_{n-1}$}$\}$
\EndIf
\EndProcedure
\end{algorithmic}
\end{algorithm}

As we can see, this solution is $\mathcal{O}(3^n)$ with respect to $\mathbf{\hat P}$ - not a very efficient solution to finding an update. Clearly, if we want to find the direction in which to update $\mathbf{g}$, we need to be more careful about how we perform the search.

During backpropogation we perform gradient descent on a differentiable function with respect to its parameters for a specific set of inputs. In adversarial testing, we do gradient descent on a loss function with respect to the input(s) at a fixed parameter setting. Suppose we have a differentiable vector function $\mathbf{\hat P}: \mathbb{R}^m\rightarrow\mathbb{R}^n$, defined as follows:
%
