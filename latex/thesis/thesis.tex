...
This can be realized by updating the parameters of $G$ and $D$ using gradient descent:

\begin{equation}
\theta_D \leftarrow \theta_D + \nabla_{\theta_D}\frac{1}{m}\sum_{i=1}^m\Big[\log D\Big(\mathbf x^{(i)}\Big) + \log\Big(1 - D\Big(G\Big(\mathbf z^{(i)}\Big)\Big)\Big)\Big]
\end{equation}

\begin{equation}
\theta_G \leftarrow \theta_G - \nabla_{\theta_G}\frac{1}{m}\sum_{i=1}^m \log\Big(1 - D\Big(G\Big(\mathbf z^{(i)}\Big)\Big)\Big)
\end{equation}
