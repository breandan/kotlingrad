...
%
\begin{equation}
    P_k(x) = \begin{cases} p_0 \circ x = x &\text{if } k=0\\ p_k\circ P_{k-1} \circ x&\text{if } k > 0 \end{cases}
\end{equation}
%
...
\end{figure}
%
Consider \autoref{fig:pbt_comparison}, which portrays the log difference between various forms of computational differentiation (evaluated using standard 32-bit precision) and AP (computed to 30 significant figures). To calculate AP, we symbolically derive the function and numerically evaluate it using \hyperref[sec:fdm]{finite difference approximation} and MacLaurin series expansion of sine and cosine. Property-based testing shows us how to compare algorithmically distinct but semantically equivalent differentiation algorithms.
...
\end{pclisting}
%
To refer to the container, we can either use \inline{18f13bb4571a} or the designated name (i.e. \inline{shady\_giraffe}). This image repository will be called \inline{user/duck}, and has an optional version identifier, \inline{:v2}, which can be pushed to the \href[subsec:docker_hub]{Docker Hub} registry:
