...
In this Chapter~\ref{ch:difftest} we present preliminary work in adversarial fuzz testing, building on prior literature in adversarial learning, metamorphic and property-based testing. A similar technique is used for testing the numerical correctness of Kotlin$\nabla$'s implementation. We present a simple algorithm for property-based shrinking using projected gradient descent and suggest several future directions for improvement.
...
\end{tikzpicture}
\caption{Unique downloads of Hatchery between the time of its release and now.}
...
The ROS middleware provides several language front-ends for polyglot programming. According to one community census in 2018, 55\% of ROS applications on GitHub are written in C/C++, followed by Python at around 25\%~\citep{Areserio54:online}. Source code for a typical ROS application will include a mixture of C/C++ and Python code, roughly corresponding to the language preferences in the robotics and machine learning communities. Hatchery can be used with most common ROS client libraries, including rosjava for Java, rospy for Python, roscpp for C/C++, and others.

\begin{figure}
\centering
\includegraphics[width=0.90\textwidth]{rqt_dep_graph.png}
\caption{A typical ROS application contains a large graph of dependencies.}
\end{figure}

A typical ROS application consists of several components, including the source code, configuration files, build infrastructure, compiled artifacts and the deployment environment. To build a simple ROS application, a number of steps are necessary. First, one must install the ROS system, which is only officially supported on Debian-based Linux distributions.

Assuming ROS has previously been installed on the host, it can be sourced like so:
...
As the publisher writes messages to \inline{channel}, the subscriber can read them off like so:
...
All ROS packages have launch file, which contain a manifest of the nodes:
...
The dominant medium for programming today is still ASCII-based, although machines still use some form of low level assembly code for execution. A great deal of software infrastructure is dedicated to translating between such representations via programming languages and compilers. Many developer tools provide a minimal command line interface (CLI), in addition to more sophisticated text editors for manipulating programs. Yet these tools are still fairly restrictive. In the same way that early computer scientists probably did not fashion a new algorithm with a sequence of holes in mind, ASCII is also an indirect representation, albeit one slightly less contrived. Language designers have tried to provide a notation is compact, familiar to use and easy to reason about execution. But most programming is still confined to text.
...
%
\begin{figure}
\begin{unbreakablelisting}[caption={A basic Kotlin$\nabla$ program with two inputs and one output, rendered below.}, label={lst:basic_kotlingrad}]
...
\end{figure}
%
In Listing~\label{lst:edsl}, we define a function with two variables and take a series of partial derivatives with respect to each variable. The function is numerically evaluated on the interval $(-1, 1)$ in each dimension and rendered in 3-space. % We can also plot higher dimensional manifolds (e.g.\ the loss surface of a neural network), projected into four dimensions, and rendered in three, where one axis is represented by time.
%
\begin{figure}
\begin{kotlinlisting}
...
\begin{tabular}{@{}c@{}}$\frac{da}{db}$\\$a'(b)$\end{tabular}  & \texttt{a.diff(b)}                                                             &  \texttt{grad(a)[b]}                                                                    & \texttt{d(a) / d(b)}                                                                       & $                    (\texttt{a}: C(\mathbb{R}^{m})^{*}, \texttt{b}: \mathbb{R}\rightarrow\mathbb{R}) \rightarrow (\mathbb{R}^{m}\rightarrow\mathbb{R})                               $ \\ \hline
    ...
    \end{tabular}
\caption{\label{tab:shape_system}Kotlin$\nabla$'s shape system species the output shape for matrix arithmetic.}
\end{table}
}
...
sealed class `100`(open val i: Int = 100): `99`(i) { companion object: `100`(), Nat<`100`> }
\end{kotlinlisting}
%
...
%
\begin{kotlinlisting}[showstringspaces=false]
val x = Var("x")
val y = Var("y")
...
In this chapter, we explore the intersection between adversarial testing and the notion of fuzz testing in software engineering. In particular, we show how probabilistic sampling with constrained optimization can be seen as an extension of property-based testing for adversarial training of differentiable programs.
...
The trouble is, we need an oracle. This is known as the test oracle problem. Furthermore, even if we had an oracle, if the space of inputs is large, it can take a long time to find an input where they disagree. Furthermore, since a single call to \inline{program(i)} can be be quite expensive in practice, this method can be inefficient.
...
The trouble is, finding the right properties to test can be quite tricky in practice, and requires a lot of effort and domain expertise. What if there were a better way?

\subsection{Metamorphic testing}

Metamorphic testing is a fuzz testing methodology which addresses the test oracle problem, and specifically the challenge of designing  low-cost domain-specific properties. It has been successfully applied in testing driverless cars~\citep{zhou2019metamorphic, pei2017deepxplore, tian2018deeptest} and other stateful deep learning systems~\citep{du2018deepcruiser}.

Roughly speaking, it can be expressed as follows:

Given an oracle $\mathbf P: Q \rightarrow S$, a set of inputs $X = \{\mathbf{x}^{(0)}, \dots, \mathbf{x}^{(z)}\}$ and outputs $\{\mathbf{y}^{(0)} = \mathbf{P}(\mathbf{x}^{(0)}), \dots, \mathbf{y}^{(z)} = \mathbf{P}(\mathbf{x}^{(z)})\}$, a metamorphic relation is a relation $\mathcal R \subset Q^z \times S^z$.

Given a function $\mathbf{\hat P}$ and a relation $\mathcal R$, a metamorphic relation over that should hold for all input-output pairs, regardless of their value. Suppose we know that $\mathbf P(\mathbf x) = \mathbf y$. Assume there exists a relation \inline{r(x)} such that $\forall \mathbf z \in \mathbf r(\mathbf x), \mathbf P(\mathbf z) = \mathbf y$. If we can find an $\mathbf r$ with a sufficiently large cardinality, then given a small set of $\mathbf x, \mathbf y$ pairs from an oracle, we can obtain a much larger set of test inputs.

For example, if we have a function that should predict the correct steering angle for an image of a road. Suppose we are given a single image and the ground truth steering angle. One metamorphic relation could be changes to lighting, small rotations, shifts or skews of the image. We should be able to apply any single relation or permutation of these relations, and the output ought to remain unchanged. If the output is drastically different, we have detected an error.

TODO: code

For adequate coverage, we need to obtain labeled pairs $\mathbf{X} = \{\mathbf{x}^{(0)}, \dots, \mathbf{x}^{(z)}\}, \mathbf{Y} = \{\mathbf{y}^{(0)} = \mathbf{P}(\mathbf{x}^{(0)}), \dots, \mathbf{y}^{(z)} = \mathbf{P}(\mathbf{x}^{(z)})\}$. Now this is starting to sound familiar!
...
%
\begin{enumerate}
\item[] \inlineimg{laptop_icon.png} &  Either commands to be run on a laptop, or ouput received thereof. \\
\item[] \inlineimg{docker_icon.jpg} &  Either \inline{Dockerfile} or Docker Compose syntax. \\
\item[] \inlineimg{raspi_icon.png}  &  Commands which should be run on a Raspberry Pi. \\
...
This procedure is identical to the snapshot technique performed in Sec. ~\ref{subsec:creating-an-image-snapshot}, but the result is cleaner. Rather than maintaining a 869 MB image, we can just store the 4 KB text file instead. To run the resulting image, we can simply use the same command as before:
...
\begin{figure}
    ...
    \caption{Browser interface for individual Duckiebots. It is provided by Portainer\footnote{For further details about Portainer and its features, visit: \url{https://www.portainer.io/}}, a wrapper for the Docker CLI which has support for container management and terminal emulation.}
...
One problem encountered during the development of Duckietown's Docker infrastructure was the matter of whether to package source code directly inside the container, or to store the sources externally (e.g. as described in \ref{subsec:volume_sharing}). If stored externally, a developer can share the sources in a shared volume and build the artifacts on container startup. Both approaches ensure reproducible build dependencies, but including the the source code makes build artifacts are standalone decipherable.
...
\caption{An early prototype of the Docker image hierarchy.}
\end{figure}
