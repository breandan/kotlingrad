...
%With these basic ingredients, we have almost all the features needed to build an expressive shape-safe AD, but unlike prior work using Scala or Haskell, in a language fully interoperable with Java, and which compiles to JVM bytecode, JavaScript, and native code.
...
%
\begin{enumerate}
    \item \textbf{Manual differentiation}: manually differentiate common functions and numerically evaluate a subset of the domain with AD.
...
Dependency hell does not just occur inside individual software projects, but across projects and development environments. Hundreds of package managers have been developed for various operating systems, programming languages, and development frameworks. Ubuntu has the Advanced Package Tool (\texttt{apt}), macOS has Homebrew (\texttt{brew}), Windows has Chocolatey (\textit{choco}). Most programming language ecosystems have their own bespoke package managers; Conan for C/C++, Maven for Java, and Cabal for Haskell. Python has developed several overlapping solutions for package management, including pip, Anaconda, PyEnv, Virtualenv, and others. Some of these install system-wide packages, and others provide command line environments. Over the lifetime of a computer system, as more packages are installed it becomes more complicated to keep track these dependencies and their effects on system behavior.

The problem basically stems from the requirement that no two versions of the same dependency can be installed simultaneously. In addition, software installers tend to spray files across the file system, which can become corrupted, are difficult to completely remove and affect future runtime behavior. To address these issues, some notion of "checkpointing" is required, so that when software applications are installed, any changes can be traced and reverted. Backups would do the job, but are cumbersome to manage and are unsuitable for development purposes. Rather, it would be convenient if there were some toolkit that allowed applications to specify a private file system, install their dependencies from tabula rasa, and be deployed as a single artifact.

\section{Operating systems and virtualization}\label{sec:os-and-virtualization}

With the growth of developer operations (\textit{devops}) a number of solutions emerged for building and running generic software artifacts. Most generically are emulators, which effectively simulate the physical circuitry of a foreign processor architecture, and thereby any software which runs on it. Another solution was virtual machines (VMs), a form of isolated runtime environment which use a \textit{hypervisor} to mediate access to hardware, but otherwise run software on physical hardware. The downside of both methods is their efficiency. Virtual machines contain full-fledged operating systems and are cumbersome to run and debug, especially to build or run a small application on a foreign OS, as is often their use case.
...
\begin{figure}[ht]
    ...
    \caption{Containers live in user space. By default they are sandboxed from the host OS and sibling containers, but unlike VMs, share a common kernel with each other and the host operating system. All system calls are passed through host kernel.}
