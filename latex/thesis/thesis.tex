...
\noindent Sometimes a function has the property that given an input $a$, no matter $a$ is changed, the output remains the same. We say that such functions have zero gradient for that input.
%
\begin{equation}
...
In this chapter, we will explore the intersection between adversarial testing and the notion of fuzz testing in software engineering. In particular, we show how probabilistic sampling with constrained optimization can be seen as an extension of property-based testing for adversarial training of differentiable programs.
