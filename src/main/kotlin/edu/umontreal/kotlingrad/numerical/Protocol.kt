...
import edu.umontreal.kotlingrad.functions.ScalarFun.ScalarVar
...
abstract class Protocol<X: RealNumber<X, Y>, Y> where Y: Number, Y: Comparable<Y> {
  ...
  infix operator fun ScalarFun<X>.plus(number: Number) = this + const(wrap(number))
  infix operator fun Number.plus(fn: ScalarFun<X>) = fn.const(wrap(this)) + fn

  infix operator fun ScalarFun<X>.minus(number: Number) = this - const(wrap(number))
  infix operator fun Number.minus(fn: ScalarFun<X>) = fn.const(wrap(this)) - fn

  infix operator fun ScalarFun<X>.times(number: Number) = this * const(wrap(number))
  infix operator fun Number.times(fn: ScalarFun<X>) = fn.const(wrap(this)) * fn

  infix operator fun ScalarFun<X>.div(number: Number) = this / const(wrap(number))
  ...
  @JvmName("infixFunPowNum") infix fun ScalarFun<X>.pow(number: Number) = pow(const(wrap(number)))
