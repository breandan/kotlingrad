...
Modern parsers are seldom handwritten unless the language is very simple or raw performance is desired. Even for source code editing, where incremental parsing and error-recovery is highly important, modern metacompilation toolkits such as ANTLR~\citep{parr1995antlr}, or Xtext~\citep{eysholdt2010xtext} cover a surprising number of common use-cases. Hatchery uses Grammar-Kit, a language toolkit designed to assist users developing custom language plugins for the IntelliJ Platform. It uses JFlex~\citep{klein2001jflex}, a DFA-based lexer and its own formal grammar, based on the parsing expression grammar (PEG), which is easily translated to the IntelliJ Platform's internal abstract syntax tree (AST) representation, the program structure interface (PSI).
...
Hatchery is capable of parsing URDF, package and launch XML, and \inline{.msg}/\inline{.srv} files.
...
\end{enumerate}
%
Most array programming libraries such as NumPy~\citep{van2011numpy} or TensorFlow~\citep{abadi2016tensorflow} take the first or second route. In Kotlin$\nabla$, we adopt the third strategy, which allows an incremental type checker, such as those typically found in IDEs, to immediately detect when a matrix operation is invalid. Consider the following example:
%
\begin{kotlinlisting}
...
\end{kotlinlisting}
%
A similar technique can be found in nalgebra~\citep{crozet2019nalgebra}, a shape-checked linear algebra library for the Rust language which, like Kotlin$\nabla$, also uses synthetic type-level integers. This technique originates in Haskell, a language which supports more powerful forms of type-level computation, such as \textit{type arithmetic}~\citep{kiselyov2005number}. Type arithmetic would make it easy to express convolutional arithmetic~\citep{dumoulin2016guide} and other arithmetical operations on shape variables, which is currently not possible in Kotlin$\nabla$, or highly impractical as it would require enumerating every possible combination of type literals in its type system.
