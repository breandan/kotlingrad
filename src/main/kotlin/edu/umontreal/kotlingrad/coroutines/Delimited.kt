...
fun <T> reset(body: suspend DelimitedScope<T>.() -> T): T =
    DelimitedScopeImpl<T>().also {
        body.startCoroutine(it, it)
    }.runReset()
...
operator fun <T, R> DelimitedContinuation<T, R>.invoke(value: R): T =
    invokeWith(Result.success(value))
...
private class DelimitedScopeImpl<T> : DelimitedScope<T>(), Continuation<T>, DelimitedContinuation<T, Any?> {
    ...
    fun runReset(): T {
        while (true) {
            val shifted = shifted?.also { shifted = null }
                ?: return takeResult()
