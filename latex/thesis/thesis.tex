...
\end{kotlinlisting}
%
The trouble is, someone needs to sit down and write a bunch of test cases for this strategy to catch any bugs.
...
\end{kotlinlisting}
%
The trouble is, we need an oracle, an often unreasonable assumption. This is known as the \textit{test oracle problem}. Furthermore, even if we had an oracle, if the space of inputs is large, it can take a long time to find an input where they disagree. Furthermore, since a single call to \inline{program(i)} can be be quite expensive in practice, this method can be inefficient.
...
\end{kotlinlisting}
%
Roughly speaking, \inline{shrink} attempts to find the minimal counterexample. For example, if \inline{program} has type \inline{(Float) -> T}, a na\"ive shrinker might be implemented as follows:
%
\begin{kotlinlisting}
...
For example, suppose our MR is $\forall \mathbf\varphi \in ||\mathbf\varphi|| < \varepsilon, \mathbf P(\mathbf x) \approx \mathbf P(\mathbf x' = \mathbf x + \varphi) \approx \mathbf y$. Given a program $\mathbf{\hat P}$ and small set of inputs and outputs from an oracle $\langle \mathbf X, \mathbf Y\rangle = \{\langle \mathbf x_0, \mathbf y_0 \rangle, \langle \mathbf x_n, \ldots, \mathbf y_n \rangle\}$, the MR suggests an $\mathbf X', |\mathbf X| \ll |\mathbf X'|$ on which to test $\mathbf{\hat P}$, without requiring corresponding outputs from $\mathbf P$. If we can find $\mathbf x \not\sim_{\mathcal R} \mathbf x'$, then we can infer at least one of the following:

\begin{enumerate}
\item $(\langle \mathbf x, \mathbf P(\mathbf x)\rangle, \langle \mathbf x', \mathbf P(\mathbf x')\rangle) \notin \mathcal R$, i.e. the MR is invalid
\item $\mathbf{\hat P}(\mathbf x) \not\approx \mathbf{P}(\mathbf x')$, i.e. the program under test is broken
\end{enumerate}

Consider a more concrete example: suppose we have implemented a program which takes an image from a vehicle while driving, and predicts the simultaneous steering angle of the vehicle. Given a single image and the corresponding ground-truth steering angle from an oracle (e.g. a human driver or simulator), a metamorphic relation one might use is approximate invariance under a set of image transformations, such as limited illumination changes, linear transformations or noise corruption. Intuitively, the steering angle should remain approximately constant, regardless of any single transformation or sequence of transformations on the original image which satisfy the chosen metamorphic relation. If not, this indicates our program is not sufficiently robust and may not generalize well.
%
\begin{kotlinlisting}
...
But this would be very inefficient. Instead, we should have a more principled way to mutate the input so as to find invalid outputs.
