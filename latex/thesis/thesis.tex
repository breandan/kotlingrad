...
Neural networks provide a powerful set of gradient-based function approximators, which are surprisingly effective compared to many handcrafted algorithms. But the same methods which have proven so effective in practice are also prone to failure when presented with carefully designed inputs. Furthermore, trained models are notoriously difficult to transfer to new domains where the training and test distributions are dissimilar. But the same gradient-based optimization techniques used to train neural networks can also be employed to probe the failure modes of neural networks and differentiable programs.

In this chapter, we will explore the intersection between adversarial testing and the notion of fuzz testing in software engineering. We will see how the notion of software testing and adversarial methods in machine learning share a curious resemblance. In particular, we show how probabilistic sampling with constrained optimization can be seen as an extension of property-based testing for adversarial training of differentiable programs, and propose PBT algorithm which incorporates features of probabilistic programming and gradient-based optimization.
