...
This leads us to adversarial testing. In adversarial testing, we generally want to find a small change to the input of a function that produces a large change to its output. Imagine we would like to change the inputs of a function $\mathbf{\hat P}: \mathbb R^m \rightarrow \mathbb R $ by a small amount $c \in \mathbb R$, so as to produce the largest output. Suppose that for each input parameter $g_0, g_1, \ldots, g_{m-1}$, we have one of three choices to make: either increase the value by a fixed amount, decrease the value by the same amount, or leave it unchanged. We are given no further information about $\mathbf{\hat P}$. Consider the na\"ive solution, in which we try every option and select the inputs corresponding to the greatest output:
...
Assume that $\mathbf{\hat P}$ is differentiable almost everywhere. Consider the following refinement of algorithm \autoref{alg:naive_fuzz}, which uses a finite difference method.
...
We now have a procedure that is $\mathcal{O}(n)$ with respect to $\mathbf{\hat P}$, but will only give us local improvements - we can still do better. Furthermore, we have not yet incorporated the notion of search constraints. Perhaps we can combine the notion of metamorphic testing seen in \autoref{subsec:metamorphic-testing} with gradient descent to accelerate the search for adversarial examples.
...
Our largest single contribution to the Duckietown project involved writing a script for flashing bootable media with a DuckieOS image. Prior to its creation, installation was a manual and time-consuming process. Afterwards, the following command is all that is needed:

\begin{dtslisting}
dt> init_sd_card [--hostname "DUCKIEBOT_NAME" --wifi "username:password"]
\end{dtslisting}
%
Users insert an SD card and follow the instructions provided. When complete, the SD card must be removed and inserted it Raspberry Pi's SD card slot on the Duckiebot. On first boot, care must be taken to ensure the device is powered continuously for a minimum of ten minutes.
