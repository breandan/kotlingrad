...
\section{Landmarks in the software development lifecycle}

Tranditional software engineering has followed a rigorous process model and testing methodology.

\subsection{Requirements}
\subsection{Design}
\subsection{Implementation}
\subsection{Verification}
\subsection{Maitenance}

\section{Developers, Operations, and the DevOps toolchain}

Recently, software engineers have started to realize the importance of bespoke tools that facilitate the process of shipping software, in addition to the software itself.
...
\section{Continuous Delivery and Continual Learning}

In CI/CD, software artifacts are "continously" updated. Similarly, intelligent systems continously adapt to their environment and will continue to accelerate this process.

Incremenental updates will grow increasingly smaller, until the program starts to after every input it processes.

We need tools to more effectively harness the stochasticity of these learning systems.

\chapter{Programming tools for robotics systems}

Programming tools are a bicycle for the mind. Complex interconnected systems need better tools for developers.
...
Most machine learning and scientific computing is written in dynamic languages, like Python. In contrast, most of the industry uses statically typed languages.

Dynamically typed languages are mostly for experimentation and prototyping. But is it scaleable to production systems?
...
\section{Differentiable programming}
    ...
    The renaissance of modern deep learning is widely credited to progress in three research areas: algorithms, data and hardware. Among algorithms, most researchers have focused on deep learning architectures and representation learning. Equally important, arguably, is the role that automatic differentiation (AD) has played in facilitating the implementation of these ideas. Prior to the emergence of general-purpose AD libraries like Theano, PyTorch and TensorFlow, gradients needed to be programmed manually. The emergence of these and other libraries specifically tailored to deep learning simplified and accelerated the pace of machine learning, allowing researchers to focus on gradient-based optimization, network architectures and learning representations. Some of these ideas in turn, formed the basis for new methods in automatic differentiation, which continues to be an active area of research.

    In deep learning, research primarily focuses on differentiable representations. In contrast, many problems we would like to solve are non-differentiable in their natural domain. For example, the structure of character-based representations are not easily differentiable as small changes to a word's symbolic representation can cause large differences to its semantic meaning. Research has shown the key to solving many classes of problems requires posing a good representation. For example, if we can represent words as a vector of real numbers, so that relations between words are spatially preserved by the vector representation, then we can perform gradient descent on such a space. Many classes of discrete problems can be relaxed to a continuous surrogates by introducing a good representation.

    As more representations and architectures emerged, researchers observed the connectionist models were part of a broader class of differentiable architectures that could be designed and interpreted in a manner not unlike programming [1], [2]. Hence the term \it{ differentiable programming} was born.
