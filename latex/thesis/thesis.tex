...
One problem encountered during the development of Duckietown's Docker infrastructure was the matter of whether to package source code directly inside the container, or to store the sources separately (e.g. as described in \ref{subsec:volume_sharing}). If the sources are stored externally, a developer can use a shared volume to build her artifacts. Both approaches ensure reproducible build dependencies, but including the the source code makes build artifacts are standalone decipherable.
