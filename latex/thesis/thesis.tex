...
The numeric tower also allows us to define common behavior on abstract algebraic structures, which are easily extended to concrete number systems. For example, to later define a field over the complex numbers or other algebras,~\footnote{ex. In order to calculate derivatives in a quaternion neural network.\citep{isokawa2003quaternion}} one must simply extend the numeric tower and override the default implementation. Most mathematical operations can be defined by composing a small set of primitive operations, which can later be differentiated in a generic fashion, rather than on an ad hoc basis.
...
Roughly speaking, during backpropogation we perform gradient descent on a differentiable function with respect to its parameters for a specific set of inputs. In adversarial testing, we do gradient ascent on a loss function with respect to the input(s) at a fixed parameter setting.
...
\section{Background}
%
\noindent Suppose we have a differentiable vector function $\mathbf{\hat P}: \mathbb{R}^n\rightarrow\mathbb{R}$:
%
...
\begin{algorithm}
    ...
    \label{alg:prob_fuzz}
    \begin{algorithmic}[1]
        \Procedure{ProbFuzz}{$\mathbf{\hat P}(\mathbf\Theta)$, budget, S, shrink = False}
        \State $D_T \gets []$
        \State $count \gets 0$
        \While{$count \le budget$}\Comment{Iterate until count exceeds our budget}
            \State $\mathbf{x}^{(i)} \sim S$\Comment{Sample from S}
            \If {$P(\mathbf{x}^{(i)})\implies \neg T$}
                \State append DiffShrink$(\mathbf{\hat P}(\mathbf\Theta), \mathbf{x}^{(i)}, false)$ to $D_T$
                \State \textbf{continue}
            \EndIf
            \State append DiffShrink$(-\mathbf{\hat P}(\mathbf\Theta), \mathbf{x}^{(i)}, true)$ to $D_T$
            \State $count++$
        \EndWhile
        \State \Return $D_T$
        \EndProcedure
...
\begin{algorithm}
    ...
    \label{alg:gd_shrink}
    \begin{algorithmic}[1]
        \Procedure{DiffShrink}{$\mathbf{\hat P}(\mathbf\Theta)$, $\mathbf{x}^{(i)}$, passingStatus}
        \State $n \gets 0$
        \State $\mathbf{x}^{(i)}_n = \mathbf{x}^{(i)}$
        \While {$n \leq C$ \textbf{and} $\neg converged$}
            \State $n++$
            \State $\mathbf{x}_{n}^{(i)}=\mathbf{x}_{n-1}^{(i)} - \alpha\nabla \mathbf{\hat P}(\mathbf\Theta)$
            \If {$T\left(\mathbf{x}^{(i)}_n, \mathbf{\hat P}(\mathbf{x}^{(i)}_n;\mathbf\Theta)\right) \neq passingStatus$}
                \State \Return $\mathbf{x}^{(i)}_n$
            \EndIf
            \State $n++$
        \EndWhile
        \State \Return none
...
Algorithm for finding test failures. First select a candidate input $\mathbf{x}^i$ according to sampling strategy $S$ (e.g.\ uniform random, or a neural network which takes $\mathbf{\hat P}$ and $T$ as input). If $\mathbf{\hat P}(\mathbf{x}^i; \mathbf \Theta)$ violates $T$, we can append $x^i$ to $D_T$ and repeat. Otherwise, we follow the gradient of $\mathcal{L}(\mathbf{\hat P}, \mathbf{x})$ with respect to $\mathbf{x}$ and repeat until test failure, gradient descent convergence, or a fixed number of steps $C$ are reached before resampling $\mathbf{x}^{i+1}$ from the initial sampling strategy $S$ to ensure each gradient descent trajectory will terminate before exhausting our budget.
