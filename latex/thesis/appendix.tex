...
%
Now consider the multivariate case, where $\mathbf w, \mathbf x \in \mathbb R^2$:
%
\begin{equation}
...
\end{equation}
%
Here, to represent $\mathbf{\hat f}$ we need to consider the pairwise interactions between every $\mathbf w$ and $\mathbf x$ at every degree of the polynomial. Instead of doing so explicitly, let us construct a special function $k: \langle\varphi(\mathbf x), \varphi(\mathbf w)\rangle \mapsto \mathbb R$, satisfying $k(\mathbf w, \mathbf x) = (1 + \mathbf w^\intercal \mathbf x)^2$. Called a \textit{quadratic kernel}, this function is part of a well-studied family known as \textit{kernel functions}, $K: \Omega \times \Omega \rightarrow \mathbb R$ with certain properties. Skipping the details, designing a valid kernel preserves the inner product in feature space. Letting $\varphi(\mathbf v) = \left[1, v_1^2, v_2^2, \sqrt{2} v_1, \sqrt{2} v_2, \sqrt{2} v_1 v_2\right]$:
%
...
\end{align}
%
Observe that by expanding the kernel, $k(\mathbf w, \mathbf x)$ as follows:
%
\begin{align}
...
\end{align}
%
We recover \autoref{eq:feature_map}, without directly computing or even needing to remember $\varphi$. This is called the \textit{kernel trick}, a widely used technique in many areas of machine learning.
