...
One common source of software variability are software dependencies. Operating systems have struggled with managing software dependencies for many years before it was discovered that the complexity of dependency resolution was NP-complete~\cite{abate2012dependency}. If we assume that no two version of the same package can be installed at the same time, then for some set of software packages a user wishes to install, and dependencies which are required to install them, determining the latest version of the dependencies which satisfy all packages can be reduced to 3-SAT and is as hard as the hardest problems in NP. This problem is known informally as "dependency hell".
