...
\centering
\begin{tikzpicture}
\begin{axis}[title={Average loss curves for polynomial regression using momentum SGD}, width=\textwidth, height=7cm, xlabel=Epochs, ylabel=$t_0$-normalized MSE, legend pos=north east, align=center]
...
\end{tabular}
\caption{\label{tab:oracle_vs_model} Above: Ground truth and trained model predictions for a single expression. Below: A single particle attacks the model by seeking high error on the surrogate loss.}
\end{table}
