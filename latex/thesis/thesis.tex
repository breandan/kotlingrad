...
The essence of refactoring is renaming. Consider what needs to occur when a user wants to rename a token in a program, such as the \inline{data} parameter on line \#1 below:
...
If they were using \inline{vim}, one solution would be to replace all textual occurrences of the string \inline{data} in the file via \inline{:\%s/data/msg/g}, with the following result:
...
\begin{pythonlisting}
def callback((*\cfbox{red}{\texttt{data}}*)):
    rospy.loginfo(rospy.get_caller_id() + "received data: %s", (*\cfbox{red}{\texttt{data}}*).data)
\end{pythonlisting}
