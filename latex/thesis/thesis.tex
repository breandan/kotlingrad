...
Realistically, machine learning needs better ways to practice its skills and probe the effectiveness of a candidate solution within a limited computational budget, ideally without harming anyone in the process. The goal of testing is to highlight errors, but ultimately to provide feedback to the system. In software engineering, the real system under test is the ecosystem of humans and machines which provide each other's means of subsistence. Critical to this arrangement that there is an external testing mechanism which enforces a minimum bar of rigor, typically some form of user acceptance testing or real-world data. If the testing mechanism is not somehow opposed to the system under test, an intelligent system can deceive itself, which is neither in the system's nor its users' best interests.

\subsection{Software maintenance and reproducibility}

Applied machine learning shares many of the same development challenges as traditional software maintenance, with source code, dependency and release management. We can imagine the current state of training a new deep learning model as a very long compilation step that requires a lot of trial-and-error. However, it differs from traditional software maintenance in that the source code is a very high level language which does not directly correspond to the computation being performed, but rather is a kind of meta-meta-program. The first meta-program is a language which configures a large directed acyclic graph. The second meta-program is the graph itself, which tries to reconstruct a program which we do not have access, save for a few input-output pairs. Emerging techniques in meta-learning like differentiable architecture search~\cite{liu2018darts} add even further layers, by optimizing over the structure of the computation graph itself.

Hardware manufacturers are busy developing a variety of custom silicon for training and inference with specialized deep learning graphs. But unlike general purpose programming, deep-learning is a fundamentally basic model of computation. As long as a computer can add and multiply, it has the capability to train and run a deep neural network. Yet due to the variety of hardware platforms and associated software churn, many machine learning models can be frustratingly difficult to reproduce on new hardware, even with the same source code and dependencies. While many claim portability, if developers are not careful about how we design our models, they may simply not run, or produce very different results when run on different hardware. Furthermore, most platforms are produced by competing industry vendors, with competing chips and incompatible standards (e.g. MLIR, ONNX, NNEF, OpenVINO, et al.). While some have tried to leverage existing compiler frameworks such as GHC~\cite{elliott2018simple} or LLVM~\cite{wei2017dlvm}, there is small sign of convergence.

In order to mitigate the effects of software variability and enable the deployment of intelligent software systems to heterogeneous platforms, we turn to a developer tool called Docker, part of a broader set of tools for build automation and developer operations. Docker allows developers to freeze a software system from their development environment, and have another developer (e.g. on a different environment) instantaneously reproduce their work on their hardware. While Docker is a technical solution, it also encourages a set of best practices for shipping software which are more process-related.
