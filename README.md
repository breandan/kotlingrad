...
|       **A** + **1**<br>**A** - **1**       |    `a + one`<br>`a - one`     |          `++a`<br>`--a`          | `a++`,`a.inc()`<br>`a--`,`a.dec()`  |                     (ℝ<sup>#</sup>→ℝ<sup>M×M</sup>) → (ℝ<sup>#</sup>→ℝ<sup>M×M</sup>)                      |
|      *sin*(a)<br>*cos*(a)<br>*tan*(a)      |                               | `sin(a)`<br>`cos(a)`<br>`tan(a)` | `a.sin()`<br>`a.cos()`<br>`a.tan()` |                                               (ℝ→ℝ) → (ℝ→ℝ)                                                |
|                *ln*(**A**)                 |                               |       `ln(a)`<br>`log(a)`        |        `a.ln()`<br>`a.log()`        |                     (ℝ<sup>#</sup>→ℝ<sup>M×M</sup>) → (ℝ<sup>#</sup>→ℝ<sup>M×M</sup>)                      |
...
|                  √a<br>∛a                  | `a.pow(1.0/2)`<br>`a.root(3)` |      `sqrt(a)`<br>`cbrt(a)`      |      `a.sqrt()`<br>`a.cbrt()`       |                           (ℝ<sup>#</sup>→ℝ<sup>M×M</sup>) → (ℝ→ℝ<sup>M×M</sup>)                            |
