...
\begin{figure}
\begin{tikzpicture}
\begin{axis}[title={Log errors between AD and SD on $f(x) = \frac{\sin(\sin(\sin(x))))}{x} + x\sin(x) + \cos(x) + x$}, width=\textwidth, height=10cm, xlabel=$x$, ylabel=$\log_{10}(\Delta)$, legend pos=south east, align=center]
...
\end{axis}
\end{tikzpicture}
\caption{We compare numerical drift between AD and SD over a swollen expression with Double precision. AD and SD both exhibit an absolute error (i.e. with respect to infinite precision (IP)) several orders of magnitude greater than their relative error with respect to each other. This result supports the findings of ~\citet{laue2019equivalence}.\vspace{-10pt}}
...
ROS.org builds nightly ARM and x86 images for robotics development. For each distro, there are packages like \inline{core}, \inline{base}, \inline{perception} (including \href{https://opencv.org/}{OpenCV}), \inline{robot} and others.
%
\begin{centering}
