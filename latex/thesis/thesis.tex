...
The derivative, as it is commonly known today, is usually associated with the calculus of infinitesimals. But the same derivative introduced by Leibniz and Newton over three centuries ago is starting to reappear in strange and marvelous places throughout mathematics and computer science. In \citet{brzozowski1964derivatives}, which reinterprets the notion of derivative for regular expressions, we see one of the early modern applications of derivatives in a classically non-differentiable setting. Brzozowski's work has important and far-reaching applications in automata theory~\citep{berry1986regex, antimirov1996partial, champarnaud1999regular} and parsing \citep{might2011parsing, moss2014derivatives}. The notion of derivative has also arisen in lambda calculus~\citep{ehrhard2003differential} with applications in type theory~\citep{mcbride2001derivative, mcbride2008clowns} and linear logic~\citep{clift2018derivatives}. This pattern of clues is surely no mere coincidence, and suggests an unrealized connection between differential and algebraic geometry, perhaps with important insights for representation learning and differentiable programming.

The work described above provides a framework for exploring symbolic differentiation using algebraic structures like \inline{Group}, \inline{Ring}, and \inline{Field}, however our implementation of automatic differentiation primarily relies on vector fields. In future work, we would like to explore the relationship between differentiable programming and the notion of symbolic differentiation in classically non-differentiable settings such as on graphs and other discrete data structures. Perhaps there exists an analog to gradient descent which can be exploited to accelerate optimization in such spaces.
...
\State \Return $\operatorname{argmax}\{\mathbf{\hat P}(g_0 + c) \circ$\Call{BfAdversary}{$\mathbf{\hat P}(g_0 + c), g_1, \ldots, g_{m-1}$},\newline
\hspace*{10em} $\mathbf{\hat P}(g_0 - c)\circ$\Call{BfAdversary}{$\mathbf{\hat P}(g_0 - c), g_1, \ldots, g_{m-1}$},\newline
\hspace*{10em} $\mathbf{\hat P}(g_0)\circ$\Call{BfAdversary}{$\mathbf{\hat P}(g_0), g_1, \ldots, g_{m-1}$}$\}$
\EndIf
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        \Procedure{ProbAdversary}{$\mathcal L: \mathbb R^m \rightarrow \mathbb R$, $\mathcal S_n$, $\mathbf T: \mathbb R^m \times \mathbb R \rightarrow \mathbb B$, budget: $\mathbb Z^+$}
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        \Procedure{DiffAdversary}{$\mathcal L: \mathbb R^m \rightarrow \mathbb R$, $\mathbf{x}_0: \mathbb R^m$, $\mathbf T: \mathbb R^m \times \mathbb R \rightarrow \mathbb B$}
        \State $i \gets 1$
...
%
\begin{enumerate}
    \item \url{https://www.wolframalpha.com/input/?i=(y_i-((%CE%B8_1%2Bh)x_i%2B%CE%B8_0))%5E2-(y_i-((%CE%B8_1-h)x_i%2B%CE%B8_0))%5E2}
    \item \url{https://www.wolframalpha.com/input/?i=(y_i-(%CE%B8_1*x_i%2B%CE%B8_0%2Bh))%5E2%E2%88%92(y_i-(%CE%B8_1*x_i%2B%CE%B8_0-h))%5E2}
\end{enumerate}
...
\end{align}
%
Notice how we recover the same solution obtained from partial differentiation and finite difference approximation, albeit in a more compact form. For a good introduction to matrix calculus, the textbook by \citet{magnus2019matrix} is an excellent guide, of which \citet{petersen2008matrix} offer a review of important identities.
