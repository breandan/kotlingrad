...
%
\begin{kotlinlisting}
fun <T, Y> metamorphicTest(program: (T) -> Y, mr: (T, Y, T, Y) -> Boolean) {
    while(true) {
        val (input: T, output: Y) = drawSampleFromData()
        val tx: (T) -> T = mr.generateValidTransformation()
        val txInput: T = tx(input)
        val txOutput: Y = program(txIn)
