...
Oracle testing is an automated software testing methodology which automatically generates test inputs in order to break a known function. For example, consider the following test:
%
\begin{kotlinlisting}
fun simpleFuzzTest() {
...
For example, suppose that our MR is $\mathbf P(\mathbf x) = \mathbf P(\mathbf x' = \mathbf x + \varphi) = \mathbf y, \forall \varphi \in [0, \varepsilon]$. Given a program $\mathbf{\hat P}$ and small set of input-output pairs from an oracle $\langle \mathbf x, \mathbf y \rangle \in \mathbf P$, the MR suggests a much larger set of inputs $\{\mathbf x: |\mathbf x - \mathbf x'| < \varepsilon\}$ on which to test $\mathbf{\hat P}$, without requiring corresponding outputs from $\mathbf P$. If we can find $\mathbf x' \in \{\mathbf x: |\mathbf x - \mathbf x'| < \varepsilon \} \mid \mathbf P(\mathbf x) \neq \mathbf{\hat P}(\mathbf x')$, this implies $\mathcal R$ does not hold over $\mathbf{\hat P}$ and one or both of the following statements are true:
...
\item $\mathbf{\hat P} \neq \mathbf{P}$, i.e. the program under test is broken
\end{enumerate}
...
But this would be very inefficient. Instead, we should have a more principled way to mutate the input so as to find valid outputs.
