...
In this chapter, we will discuss the challenges of software reproducibility and how best practices in software engineering like continuous integration and delivery can help researchers remove some variability associated with building and running software. Docker~\cite{merkel2014docker} is one technical solution we will address.

Part of the challenge of building intelligent systems and programming at large, is the issue of reproducibility. Software reproducibility has a number of subtle aspects, including hardware compatibility, operating systems, file systems, build reproducibility, and determinism. While at one point in the history of computing, it may have been sufficient to write down a computer program in its entirety, the source code of today's programs are meaningless abstractions on their own - built by programs, interpreted by programs, and which communicate with other programs, they are essentially pebbles sitting on a pyramid of transitive dependencies. As software is updated and rebuilt, important details about the original development environment can be misplaced in the process.

The total information required to implement an program in a modern programming language requires gigabytes of data. Part of that data includes training data and experimental reproducibility, which we do not consider in this work. A significant portion of that data pertains to the software for building and running programs, including the operating system, software dependencies, drivers and development tools. These are the types of data which software reproducibility considers.

We would often like to reproduce the work of fellow researchers, but the mental effort of re-implementing their abstractions can be tedious and detrimental towards scientific progress. Similarly in programming, it is often unnecessary to reinvent the wheel, and would be convenient if we had tools for reproducibility and incremental development. This is the same challenge which software engineers have been cultivating for many years, via source code management and the open source community. But version control tools alone are insufficient, as they primarily deal with text. Programs are built and run on heterogenous platforms running all manner of software. To these sources of variability must be specifically considered when reproducing a software system.
