...
Metamorphic testing is a fuzz testing methodology, which addresses the challenge of designing domain-specific properties, by leveraging more a more generic property: metamorphic relations. It has been successfully applied in testing driverless cars~\citep{zhou2019metamorphic, pei2017deepxplore, tian2018deeptest} and other stateful deep learning systems~\citep{du2018deepcruiser}.

Roughly speaking, it can be expressed as follows: We find a relation over inputs that should hold for all input-output pairs, regardless of their value. Suppose we know that $\mathbf P(\mathbf x) = y$. Assume there is a relation \inline{r(x)} such that $\forall \mathbf z \in \mathbf r(\mathbf x), \mathbf P(\mathbf z) = y$. If we can find an $\mathbf r$ with a sufficiently large cardinality, then given a small set of $\mathbf x, \mathbf y$ pairs from an oracle, we can obtain a much larger set of test inputs.
