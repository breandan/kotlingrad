...
\begin{figure}
    ...
    \caption{Royce's original Waterfall model describes the software engineering process, but can also be found in most engineering disciplines. We use it to guide our discussion and frame our contributions inside of this model.\vspace{-10pt}}
...
The majority of existing AD frameworks for machine learning are implemented in dynamically typed languages like Python, Lua and JavaScript, with some early implementations including projects like Theano~\citep{bergstra2010theano}, Torch~\citep{collobert2002torch} and Caffe~\citep{jia2014caffe}. Similar ideas have arisen in statically-typed, functional languages, such as Haskell (\href{https://github.com/Functional-AutoDiff/STALINGRAD}{Stalin$\nabla$}~\citet{pearlmutter2008using}), Scala (\href{https://tongfei.me/nexus/}{Nexus}~\citet{chen2017typesafe}), F\# (\href{http://diffsharp.github.io/DiffSharp/}{DiffSharp}~\citet{baydin-diffsharp}), \href{https://www.tensorflow.org/swift}{Swift}~\citep{swift}, et al., but most of these are unable to check the shape of multidimensional arrays in their type system, and those which do are typically implemented in experimental languages with sophisticated type-level programming features. In our work, we demonstrate the viability of shape-checked differentiable programming in a widely-used language with parametric types. This ensures that programs on matrices, if they compile, are the correct shape and can be numerically evaluated at runtime.
...
More broadly, type checking (as explored in ~\autoref{ch:kotlingrad}) and testing can be seen as part of a broader spectrum of techniques for software verification and validation. The sooner developers can detect software anomalies, the easier they are to fix and the safer autonomous systems can become. Previous techniques in the software testing literature have called for domain specific expertise to handcraft input-output examples or otherwise constrain the search space, but with advances in adversarial machine learning, the development of automated software validation becomes an increasingly attractive proposition.
...
%
\begin{enumerate}
\item[] \inlineimg{laptop_icon.png} &  Shell commands intended for a personal computer, or output derived thereof. \\
...
\welldone{\chapter{Design: Programming tools for robotics}\label{ch:hatchery}}
\setlength{\epigraphwidth}{0.78\textwidth}
\epigraph{``The hope is that, in not too many years, human brains and computing machines will be coupled together very tightly, and that the resulting partnership will think as no human brain has ever thought and process data in a way not approached by the information-handling machines we know today.''}{\begin{flushright}--Joseph \citet{licklider1960man}, \textit{Man-Computer Symbiosis}\end{flushright}}
...
Hatchery is a tool designed to assist programmers writing robotics applications using the ROS middleware. At the time of its release, \href{https://github.com/duckietown/hatchery}{Hatchery} was the first ROS plugin for the IntelliJ Platform \footnote{An IDE platform for C/C++, Python and Android development}, and today, is the most widely used with over 10,000 unique downloads. While the idea is simple, its prior absence and subsequent adoption suggest there is unmet demand for such tools in the development of intelligent software systems, particularly in domain-specific applications like robotics.
%
...
With the development of modern languages came programming tools which were capable of representing code as a mixture of hypertext and graphical user interfaces. These tools provide a somewhat richer representation for code than plaintext and help to capture programs' graph-based structure, but ultimately use ASCII with sparse visual cues to render code. Some tools support larger character sets and font-based typographic ligatures, although the visual representation of source code remains mostly linear.
...
The Hatchery plugin provides a lightweight GUI overlaying the program's source code. This interface (\autoref{fig:hatchery_gui}) primarily consists of simple visual cues such as text highlighting, navigation assistance and other menus and configuration panels for performing various programming tasks. The host IDE offers a visual language consisting of iconography and repetitive visual motifs, which serve as cognitive aides to guide the developer's procedural memory. The IntelliJ Platform offers a palette of common design elements, which users who are familiar with the IDE can recognize at a quick glance. Plugins can use these same patterns to access procedural memories implanted in the userbase, facilitating transfer learning. To do this properly requires familiarity with the design language and careful integration with the target framework (i.e. ROS).\\
%
\begin{figure}[b]
    \centering
...
In this chapter we explore the value of IDEs for general purpose software development and present a domain specific IDE plugin for robotics development, originally developed as a final project in the Duckietown class~\citep{paull2017duckietown}. By using Hatchery, developers can receive assistance when writing, compiling and running ROS applications. The author wishes to express his gratitude to \href{https://github.com/paoloach}{Paolo Achdjian} for contributing various features.

\welldone{\chapter{Implementation: languages and compilers}\label{ch:kotlingrad}}

\setlength{\epigraphwidth}{0.90\textwidth}
\epigraph{``Although mathematical notation undoubtedly possesses parsing rules, they are rather loose, sometimes contradictory, and seldom clearly stated\textellipsis Because of their application to a broad range of topics, their strict grammar, and their strict interpretation, programming languages can provide new insights into mathematical notation.''}{\begin{flushright}--Kenneth \citet{iverson1999math}, \textit{Math for the layman}\end{flushright}}
...
\end{kotlinlisting}
%
Like \href{https://docs.python.org/3/reference/datamodel.html#special-method-names}{Python}, Kotlin supports overloading a limited set of \href{https://kotlinlang.org/docs/reference/operator-overloading.html}{operators}, which are evaluated using a \href{https://kotlinlang.org/docs/reference/grammar.html#precedence}{fixed precedence}. In the current version of Kotlin$\nabla$, operators do not perform any computation, they simply construct a directed acyclic graph representing the symbolic expression. Expressions are only evaluated when invoked as a function.
...
\end{kotlinlisting}
%
This DSL, which is used to support variable capture, can be used as follows:
%
\begin{kotlinlisting}
with(DoubleContext) {
    val t = X + Y + 0.0
...
\href{https://deeplearning4j.org}{Eclipse DL4J}                                  & Java    & \xmark & \cmark & \xmark & \cmark & \xmark & \xmark & \xmark \\
...
\href{https://github.com/google/tangent}{Tangent}                                & Python  & \xmark & \cmark & \xmark & \xmark & \xmark & \xmark & \xmark \\
...
Allowing users to specify matrix structure in its type signature, (e.g.\ Singular, Symmetric, Orthogonal) would admit specialized differentiation over such matrices (cf. section 2.8 of~\citet{petersen2012matrix} for a detailed review of specific techniques for calculating derivatives of structured matrices). \citet{makwana2018numlin} have an interesting approach for a linearly-typed encoding of linear algebra which would also be interesting to explore.
...
\welldone{\chapter{Testing and validation}\label{ch:difftest}}
...
\addplot table [mark=none, x index=0, y index=1, col sep=comma] {adsd_comparison.csv};
\addlegendentry{$\Delta$(SD, IP)}
\addplot table [mark=none, x index=0, y index=2, col sep=comma] {adsd_comparison.csv};
\addlegendentry{$\Delta$(AD, IP)}
\addplot table [mark=none, x index=0, y index=3, col sep=comma] {adsd_comparison.csv};
\addlegendentry{$\Delta$(AD, SD)}
\end{axis}
\end{tikzpicture}
\caption{We compare numerical drift between AD and SD over a swollen expression using fixed precision and infinite precision (IP). AD and SD both exhibit relative errors (i.e. with respect to each other) several orders of magnitude lower than their absolute error. These results are consistent with the findings of ~\citet{laue2019equivalence}.\vspace{-10pt}}
...
Let us consider an extension of classical fuzzing methods to differentiable functions on continuous random variables. First, we select $\mathbf{x}_j: \mathbb{R}^m \sim \mathcal S_m$ (e.g.\ uniform random prior, or using a meta-learner $m: (\mathbb{R}^m \rightarrow \mathbb{R}^n) \times (\mathbb{R}^m \times \mathbb R \rightarrow \mathbb B) \rightarrow \mathbb{R}^m$ as input). If $\mathbf{\hat P}(\mathbf{x}^i)$ violates $\mathbf T$, we can append $\mathbf x^i$ to $D_\mathbf T$ and repeat. Otherwise, we update $\mathbf x$ following $\nabla_{\mathbf x}\mathcal{L}(\mathbf{x})$ and repeat until test failure, gradient descent convergence, or a fixed number of steps $C$ are reached before resampling $\mathbf{x}$ from $\mathcal S_m$. $C$ helps to ensure each gradient descent trajectory terminates before exhausting the allotted computational budget. This procedure is described in Alg. \autoref{alg:prob_adversary}.
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \While {$i \leq I_{max}$ \textbf{and} $\mathbf ||\mathbf x_i - \mathbf x_{i-1}||_2^2 < \epsilon$} \Comment{While in budget and not converged.}
            ...
            \State $i \gets i + 1$
        \EndWhile
        \State \Return \textbf{if } $\neg t_0$ \textbf{ then } $\{\mathbf x_{i-1}\}$ \textbf{ else } $\varnothing$ \Comment{Return last iterate or $\varnothing$ if test passed.}
        \EndProcedure
    \end{algorithmic}
\end{algorithm}
...
Where $\eta$ is a common, fixed upper bound on every $l_k$. Further GAN variants such as WGAN~\citep{arjovsky2017wgan}, MHGAN~\citep{turner2019mhgan}, et al. have proposed augmentations to the traditional GAN to stabilize training and improve sample diversity. This family of techniques has also been shown successful in a number of domains such as speech synthesis~\citep{donahue2019wavegan} and graphs~\citep{wang2018graphgan}.

\section{Conclusion}

In this chapter we have visited some interesting new ideas for validating intelligent systems, from the perspective of software engineering and machine learning.  We have seen a curious resemblance between some new and old ideas in adversarial learning and automated testing, such as property-based testing, metamorphic testing, and generative adversarial networks. We have proposed a framework for evaluating differentiable programs in a low-cost, data-driven manner and suggest some directions for future research in the evaluation of differentiable programs. As today's engineers begin to incorporate learning systems in tomorrow's robotic infrastructure, we believe the increased assurance these tools provide will be an important component in the end-to-end validation of these highly-interconnected cyberphysical systems.

\welldone{\chapter{Software Maintenance and Reproducibility}\label{ch:ducker}}

\setlength{\epigraphwidth}{0.70\textwidth}
\epigraph{``An article about computational science in a scientific publication is not the scholarship itself, it is merely advertising of the scholarship. The actual scholarship is the complete software development environment and the complete set of instructions which generated the figures.''}{\begin{flushright}--\citet{buckheit1995wavelab}, \textit{WaveLab and Reproducible Research}\end{flushright}}
...
In this chapter we have taken a guided tour of containerization and demonstrated the effectiveness of container tools towards building reproducible robotics applications. We offer a set of best practices and lessons learned during the design, development and deployment of Docker containers for the Duckietown~\citep{paull2017duckietown} platform.

\mediumrare{\chapter{Case study: application for autonomous robotics}\label{ch:case-study}}
...
As a case study, we have implemented a mobile application using ROS, Docker, and Android, using the tools proposed in Chapters 2--5. The source code for this section can be found at the following URL: \url{https://github.com/breandan/duckietown-rc}

\section{Design}

Hatchery is compatible with most IDEs on the IntelliJ Platform, including \href{https://developer.android.com/studio}{Android Studio}, the official IDE for Android mobile development. To use ROS specific features from within Android Studio, ensure the Hatchery plugin is installed as described in ~\autoref{subsec:installation}. First create a new ROS Android project or clone an existing one. Then create a symbolic link inside the Android project's root directory to the location of the ROS project:
%
\begin{pclisting}
~$ ln -s /absolute/path/to/ros/project /android/project
\end{pclisting}
%
Create a new project in Android Studio and import the ROS project. Now, the ROS project can be managed from within Android Studio.\vspace{10pt}\\
%
\begin{centering}
\includegraphics[width=\textwidth]{hatchery_android.png}
\end{centering}\vspace{10pt}
%
Running nodes on non-Android devices is no different than ordinary rospy projects. However, running ROS nodes on Android devices requires \href{http://wiki.ros.org/rosjava}{rosjava}, a pure Java implementation of the ROS messaging protocol. A few \href{https://github.com/rosjava/android_apps}{example applications} provide a basic Android UI and support communicating with networked ROS nodes.

ROS Android applications can be run on a physical Android device or inside the emulator by launching an Android Virtual Device (AVD) and using the following commands, as described in the \href{https://developer.android.com/studio/run/emulator-networking#consoleredir}{Android developer documentation}:
%
\begin{pclisting}
...
Due to Android's Kotlin interoperability, Kotlin libraries are directly usable in Android projects. To use Kotlin$\nabla$ in an Android project, first add a custom Maven repository to the project's \inline{build.gradle.kts} file:
%
\begin{kotlinlisting}
repositories {
...
\end{kotlinlisting}
%
Then add the most recent stable version of Kotlin$\nabla$ to the \inline{dependencies} section of the application's \inline{build.gradle.kts} file:
%
\begin{kotlinlisting}
dependencies {
...
\end{kotlinlisting}
%
Kotlin$\nabla$ can now be used inside an Android application.
...
%
\begin{centering}
\includegraphics[width=\textwidth]{android_screenshot.png}
\end{centering}
...
\chapter{Software Maintenance and Reproducibility}
...
%
\begin{centering}
\includegraphics[width=\textwidth]{ros_docker_images.png}
\end{centering}

\subsection{\href{https://blog.hypriot.com/}{Hypriot}}
...
\subsection{\href{https://hub.docker.com/}{Docker Hub}}
...
%
\begin{centering}
\includegraphics[width=\textwidth]{docker_hub_autobuild.png}
\end{centering}
%
...
%
\begin{centering}
\includegraphics[width=\textwidth]{docker_hub_repo_links.png}
\end{centering}
%
...
\begin{centering}
\includegraphics[width=\textwidth]{docker_cloud.png}
\end{centering}
