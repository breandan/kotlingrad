...
In the early days of machine learning, it was widely believed the development of human-level intelligence simply required a sufficiently descriptive first-order logic. By accumulating a database of facts and their relations, researchers believed they could use symbolic reasoning to bypass learning, thought to be a lesser detail. This rule-based approach dominated a large portion of early research in artificial intelligence and considerable effort was poured into the collection and curation of domain-specific ontologies. Despite the best efforts of robotics, signal processing and natural language research, \textit{expert systems} were unable to scale to real-world applications, causing a great disillusionment in artificial intelligence for a number of decades. While largely unsuccessful, expert systems excelled in areas where current machine learning systems struggle such as interpretability, and there is reason to believe these ideas were simply ahead of their time.

What did finally work, is the idea of connectionist learning. By nesting random function approximators, called \textit{neural networks}, and training the whole thing via backpropagation~\cite{werbos1990backpropagation}, the system learns a surprising amount of intelligent behavior. This simple idea can be traced back to the mid-20th century, but was not fully-realized until after the widespread availability of cheap computing and large datasets. In theory, a single layer of nesting is able to approximate any continuous differentiable function~\cite{hornik1989multilayer}, but in practice learning requires deeply nesting many such approximators in sequence, hence the term, \textit{deep neural networks}. While the importance of depth was suspected for many years, the backpropagation algorithm had difficulty scaling to deeper neural networks due to the vanishing gradient problem~\cite{bengio1994learning}. Solving this problem required a number of adaptations.

The success of modern deep learning can be partly attributed to progress in automatic differentiation.

Intelligent systems engineers must think carefully about languages and abstractions. If developers are required to implement backpropogation by hand, they will have little time to think about the high-level characteristics of these systems. Similarly, if the abstractions we use have too many assumptions baked in, small variations will require costly reimplementation. This is no different from traditional software engineering - as engineers, need to choose the right abstractions for the task at hand. Too low-level and the design is lost in the details, too abstract and the details are lost completely. With machine learning, the necessity of choosing good abstractions is even more important, as the connection of between runtime behavior and source code is already stretched quite thin.
