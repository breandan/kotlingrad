...
\subection{User interface}

An often overlooked, but important aspect of development tools is the user interface, being the primary mechanism which developers use to edit source code. In early days of modern computing, the only way of getting information into or out of a computer required punching holes in paper. Later, computers were equipped with technology to emit the same binary pattern as contiguous points of light, which in aggregate, could be used to display a small alphabet. Later, through increased density and frequency, computers were able to render more sophisticated shapes and animations. This progression is the direct result of advances in video technology, but can also be viewed as improvements in program representation, where the explicit content was simply a medium for communicating developer intent and machine interpretation.

The dominant medium for writing programs today is still ASCII, but computers continue to use the same binary and machine code representations internally. A great deal of computing infrastructure is dedicated to translating between these representations via programming languages and compilers. Many developer tools provide a minimalist command line interface (CLI), in addition to more sophisticated text editors. Yet these tools are still imperfect models of a program, which is whatever the developer has inside their heads at the time of its authorship. In the same way that early computer scientists probably did not design a new algorithm with a sequence of holes in mind, ASCII is also an indirect abstraction, albeit a slightly more natural one. A good abstraction is familiar and makes it easy to translate between idea and implementation.

Modern IDEs use a mix of hypertext and graphical models. We can view this as a spectrum where on one end is text and the other is models.

In Hatchery's case, we use a lightweight graphical user interface. Most of the graphical user interface is provided by the underlying IDE, and the plugin's primary job is to integrate smoothly with the host IDE for familiarity.
