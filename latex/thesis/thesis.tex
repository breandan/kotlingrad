...
As of Docker version 18.09.6, ARM-specific \inline{Dockerfile}s will not build on x86 machines\footnote{With the exception of the Mac OS Docker client, which offers multi-architecture support. Further details on multiarch support can be found here: \url{https://docs.docker.com/docker-for-mac/multi-arch/} For }, and attempting to build one will produce the following error when running \inline{docker build}:

\begin{pclisting}
standard_init_linux.go:175: exec user process caused "exec format error"
\end{pclisting}

In order to circumvent this restriction, ARM-specific \inline{Dockerfile} can ported to run on x86 by using the \inline{RUN [ "cross-build-start" ]} and \inline{RUN [ "cross-build-end" ]} directives, after the \inline{FROM} and before the \inline{CMD} instructions.
...
To access this directory, visit the following URL: \url{http://HOSTNAME:8082/}
...
For GUI tools, run the following command on Linux:
...
The above command opens a ROS shell that is set to connect to \inline{DUCKIEBOT}'s ROS master node. To test the ROS connection works, run \inline{roswtf}. To allow incoming X connections on GUI-enabled Linux host, run \inline{xhost +} outside Docker on the same computer.\footnote{See \url{http://wiki.ros.org/docker/Tutorials/GUI#The_safer_way} for a more secure alternative.}
...
\begin{dtslisting}
dt> dts duckiebot keyboard_control DUCKIEBOT_NAME
\end{dtslisting}
