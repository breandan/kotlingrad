...
First, we enumerate a list of integer type literals as a chain of subtypes, so that $C <: C - 1 <: C - 2 <: \dots <: 1 <: 0$, where $C$ is the largest fixed-length dimension we wish to represent. Using this encoding, we are guaranteed linear growth in space and time for subtype checking. $C$ can be specified by the user, who will need to regenerate the code.
%
...
open class `100`(open val i: Int = 100) { companion object: `100`(), Nat<`100`> }
\end{lstlisting}
%
...
\end{lstlisting}
%
Users must handle all subclasses when branching on the type of a sealed class, as incomplete control flow will not compile (instead of failing silently at runtime).
%
\begin{lstlisting}[language=Kotlin]
sealed class Fun<X: Group<X>>(open val variables: Set<Var<X>> = emptySet()): Group<Fun<X>> {
    ...
    fun diff(): Fun<X> = when(this) {
       is Const -> Zero
       is Var -> One
       is Prod -> left.diff() * right + left * right.diff() // d(u*v)/dx = du/dx * v + u * dv/dx
...
\end{lstlisting}
%
Smart-casting allows us to treat the abstract type \inline{Fun} as a concrete type, e.g. \inline{Sum} after performing an \inline{is Sum} check. Otherwise, we would need to write \inline{(this as Sum).left} in order to access its field, \inline{left}. If the type were incorrect, performing a cast without checking would throw a \inline{ClassCastException}, which sealed classes prevent.
...
Extension functions augment external classes with new fields and methods. By using a technique known as context oriented programming~\cite{hirschfeld2008context}, we can expose custom extensions (e.g.\ through \inline{DoubleContext}) to consumers without requiring subclassing or inheritance.
%
...
Inspired by Stalin$\nabla$, Autograd, DiffSharp, Myia, Nexus, Lantern et al., Kotlin$\nabla$ attempts to port recent advancements in automatic differentiation (AD) to the Kotlin language. It also adds a number of novel ideas, including shape-safety, algebraic expression rewriting and numerical stability checking with property-based testing. Kotlin$\nabla$ is a symbolic graph-based autograd. \\
