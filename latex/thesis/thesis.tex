...
\noindent Likewise, for a scalar functions $Q(q_0, q_1, \dots, q_n):  \mathbb{R}^n\rightarrow\mathbb{R}$, the gradient $\nabla Q$ tells us:
...
The cost of calculating the Hessian, $\mathbf{H}$ is approximately quadratic \cite{griewank1993some} with respect to the number of independent variables under differentiation. If $\mathbf{H}(a)$ is tractable to compute and invertible, for such points we could use the second-partial derivate test to determine that:\\
