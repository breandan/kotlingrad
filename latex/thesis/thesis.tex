...
One of the most important and easily overlooked components of an IDE is the parser. Most IDEs do not use not ordinary recursive descent or shift-reduce parsers, since the source code is often invalid during use. Even when the source code is invalid, the editor is expected to recover from local errors and provide responsive feedback to users. In order to parse code while it is being edited and provide low-latency feedback to users, a special kind of parser, called an incremental parser~\citep{wagner1997practical,harper1994incremental} is needed.
