...
The Robot Operating System (ROS) is a popular middleware for robotics applications. At its core, ROS provides software infrastructure distributed messaging, shared memory and inter-process communcation, but more broadly refers to a community-developed set of programming libraries and graphical plugins which collectively form the ROS ecosystem. While it is not an operating system in the traditional sense, it provides certain features similar to operating systems. Unlike pure protcols like DDS and ZMQ, in addition to the decentralized protocol itself it provides domain-specific features for building decentralized systems, particularly those which are capable of mobility unlike similar IoT frameworks, like Android Things, Eclipse Kura, and others,

According to an open source census in 2018, 55\% of ROS applications on GitHub are written in C/C++~\cite{Areserio54:online}, followed by Python at around 25\%. This
...
In this chapter, we will discuss the challenges of software reproducibility and how containers and continuous integration can help researchers remove this source of variability. Docker~\cite{merkel2014docker} is one technical container solution we will address.

Part of the challenge of building intelligent systems and scientific programming more broadly is the issue of reproducibility. We would like to leverage the work of prior researchers, but while the mental effort of reimplementing their work can be sometimes be worthwhile, it can also be quite tedious and detrimental towards progress. Often it is unnecessary to reinvent the wheel, and would be convenient to have baselines for comparison and incremental improvement.

Fortunately, software engineers have been working on this problem for a number of years.
