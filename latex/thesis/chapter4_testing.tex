...
The adversarial testing strategy takes as input the original the regression model $\hat{f}$, a small dataset from the same distribution, and resumes the training over the provided datapoints for a fixed number of epochs, producing $\hat{f}'$. This is used to construct a surrogate loss over the input space as follows: $(\hat{f} - \hat{f}')^2$, which can be maximized in the input domain using \autoref{alg:diff_adversary}.
