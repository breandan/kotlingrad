...
In conjunction with ADTs, Kotlin$\nabla$ also uses multiple dispatch to instantiate the most specific result type of an operation based on the type of its component operands. Although multiple dispatch is not an explicit language feature, it can be emulated using inheritance and double dispatch.

Building on the previous example, imagine performing algebraic simplification, a useful trick for reducing expression swell and improving numerical stability. We can use `when` to detect the type of a subexpression at runtime, then use \textit{smart casting} to access its members afterwards, as though it were casted:
