...
\begin{algorithmic}[1]
\Procedure{ProbGen}{$\mathcal L: \mathbb R^m \rightarrow \mathbb R$, $\mathcal S_m$, $\mathbf{T}: \mathbb{R}^m \rightarrow \mathbb{B}$}
...
G_e(\delta, \psi) = \begin{cases} \text{if } \delta \leq 0 \begin{cases}
...
\begin{algorithmic}[1]
\Procedure{SurrogateAttack}{$\mathbf{\theta}: \mathbb{R}^k$, $\hat{f}: \mathbb R^k \times \mathbb R^m \rightarrow \mathbb R$, $\mathcal S_m$, $\mathbf{T}: \mathbb{R}^m \rightarrow \mathbb{B}$}
...
\State $\mathcal{\hat{L}} \gets \left(\hat{f}(\mathbf{\theta}') - \hat{f}(\mathbf{\theta})\right)^2$ \Comment{Construct the surrogate loss.}
\State \Return \Call{ProbGen}{$\mathcal{\hat{L}}, \mathcal S_m, \mathbf T$}
\EndProcedure
\end{algorithmic}
\end{algorithm}
