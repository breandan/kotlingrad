...
One of the most important and unappreciated components of an IDE is the parser. Unlike compilers, most IDEs do not use recursive descent or shift-reduce parsers, as they are fragile and not well suited for real-time editing of source code. Edits are typically short, localized changes inside a large file, and are frequently invalid. As the IDE is expected to recover from local errors and provide responsive feedback to users, re-parsing the entire document between edits would be expensive and unnecessary. In order to effectively parse source code undergoing simultaneous modification, special consideration must be taken to ensure robust and responsive parsing.

Incremental methods such as \citet{wagner1997practical} incorporate caching and other techniques to accelerate compilation and parsing. Fuzzy parsers~\citep{koppler1997systematic} aim to increase the flexibility an robustness in the presence of local errors. Both of these ideas have played an important role in the development of language-aware programming tools for analysis and compilation.
