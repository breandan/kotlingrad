...
\State \Return $\operatorname{argmax}\{\mathbf{\hat P}(g_0 + c) \circ$\Call{BfAdversary}{$\mathbf{\hat P}(g_0 + c), c, g_1, \ldots, g_{m-1}$},\newline
...
\State \Return $g_0 + \frac{\mathbf{\hat P}(g_0 - c) - \mathbf{\hat P}(g_0 + c)}{2c}$
\Else \Comment{Apply single-step gradient ascent using componentwise finite difference}
\State \Return $g_0 + \frac{\mathbf{\hat P}(g_0 - c, 0, \ldots) - \mathbf{\hat P}(g_0 + c, 0, \ldots)}{2c}$, \Call{FdAdversary}{$\mathbf{\hat P}, c, g_1, \ldots, g_{m-1}$}
\EndIf
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\begin{figure}
\centering
\includegraphics[width=0.60\textwidth]{verification_complexity.png}
\caption{Complexity of detecting various types of programming errors.}
\label{fig:verification_complexity}
\end{figure}

Type checking, compilers and automated testing are all part of a broader class of validation and verification tools. The goal of these tools is to provide feedback as early as possible. Some errors (e.g. syntactical errors), are minor nuisances and can be quickly detected with a good incremental parser. Others, as we show in~\autoref{fig:verification_complexity}, are more difficult to verify but can be detected by spending computation. We argue this computational cost is often justified as undetected bugs can have catastrophic consequences. Spending computation frees up valuable cognitive resources better spent on more challenging tasks. Studies have shown bugs detected early in the development are more likely to be fixed~\citep{distefano2019scaling} -- saving minutes during development could save lives during operation.

As today's engineers begin to incorporate learning in tomorrow's safety-critical robotic systems, we believe that the increased assurance provided by intelligent validation and verification tools will be indispensable for the deployment and scaling of these highly complex cyberphysical systems.
