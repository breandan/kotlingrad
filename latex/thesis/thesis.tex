...
One of the most important and unappreciated components of an IDE is the parser. Unlike compilers, most IDEs do not use recursive descent or shift-reduce parsers, as they are fragile and not well suited for real-time editing of source code. Edits are typically short, localized changes inside a large file, and moreover are frequently invalid. As the IDE is expected to recover from local errors and provide responsive feedback to users, re-parsing the entire document in-between edits would be expensive and unnecessary. In order to effectively parse code undergoing simultaneous modification, special consideration must be taken to ensure robust and responsive parsing.

%Incremental parser~\citep{harper1994incremental,wagner1997practical} is one strategy for handling .
