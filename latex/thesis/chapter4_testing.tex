...
\Do
    \State $\mathbf{x} \sim \mathcal S_m, \mathbf{y} \gets \mathcal{O}(\mathbf{x}), \mathcal{B}_l \gets \mathcal{B}_l - 1$ \Comment{Ask for a new label from the oracle.}
    \State $\mathbf\theta' \gets \mathbf\theta' - \nabla_{\mathbf\theta'}(\hat{f}(\mathbf{x} - \mathbf\theta')^2, \mathbf{y})$ \Comment{Update model parameters using loss gradient.}
\doWhile {$0 < \mathcal{B}_l$} \Comment{Iterate until labeling budget exhausted.}
\State $\mathcal{\hat{L}} \gets (\hat{f}(\mathbf{\theta}') - \hat{f}(\mathbf{\theta}))^2$ \Comment{Construct the surrogate loss.}
