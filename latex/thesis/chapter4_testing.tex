...
In this chapter we propose a novel property-based testing (PBT) algorithm for differentiable programs, and show an empirical improvement in sample efficiency over na\"ive probabilistic fuzzing by detecting a greater proportion of errors violating test constraints in a given budget. Our algorithm can be used to both identify trust region boundaries, and adversarially attack a pretrained model given input-output access and a few samples from the training distribution. We explore the relationship between adversarial methods in machine learning and property-based testing, and show how adversarial methods can be seen as extensions to a kind of property-based testing testing known as metamorphic testing.
...
Let us consider an extension of classical fuzzing methods to differentiable functions on continuous random variables. First, we sample an input $\mathbf{x}_j: \mathbb{R}^m \sim \mathcal S_m$ (e.g.\ uniformly). If $\mathcal{L}(\mathbf{x}_j)$ satisfies \autoref{eq:output_constraint_example}, we ascend loss following $\nabla_{\mathbf x}\mathcal{L}$, otherwise we descend and repeat until the test ``flips'', gradient vanishes, or a fixed number of steps $I_{max}$ are reached before resampling $\mathbf{x}_{j+1}$ from $\mathcal S_m$. This procedure is described in \autoref{alg:prob_adversary}.
...
\begin{algorithmic}[1]
\Procedure{ProbGen}{$\mathcal L: \mathbb R^m \rightarrow \mathbb R$, $\mathcal S_m$, $\mathbf{T}: \mathbb{R}^m \rightarrow \mathbb{B}$, $\mathcal{B}_e: \mathbb N^+$}
...
\Do
    ...
    \EndIf
\doWhile {$i \leq I_{max}$ \textbf{and} $\mathbf |\mathcal{L}(\mathbf x_i) - \mathcal{L}(\mathbf x_{i-1})| < \epsilon$} \Comment{While in budget and not converged.}
...
\begin{algorithmic}[1]
\Procedure{SurrogateAttack}{$\mathbf{\theta}: \mathbb{R}^k$, $\hat{f}: \mathbb R^k \times \mathbb R^m \rightarrow \mathbb R$, $\mathcal S_m$, $\mathbf{T}: \mathbb{R}^m \rightarrow \mathbb{B}$, $\mathcal{B}_l: \mathbb N^+$}
\State $\mathbf{\theta}' \gets \mathbf{\theta}$
\Do
    \State $\mathbf{x} \sim \mathcal S_m, \mathbf{y} \gets \mathcal{O}(\mathbf{x})$ \Comment{Ask for label from oracle.}
    \State $\mathcal{B}_l \gets \mathcal{B}_l - 1$
    \State $\mathbf\theta' \gets \mathbf\theta' - \nabla_{\mathbf\theta'}\mathcal{L}(\hat{f}(\mathbf{x}; \mathbf\theta'), \mathbf{y})$
\doWhile {$0 < \mathcal{B}_l$} \Comment{Iterate until labeling budget exhausted.}
\State $\mathcal{\hat{L}} \gets (\hat{f}(\mathbf{\theta}') - \hat{f}(\mathbf{\theta}))$
\State \Return \Call{ProbGen}{$\mathcal{\hat{L}}, \mathcal S_m, \mathbf T, \mathcal B_e$}
\EndProcedure
\end{algorithmic}
\end{algorithm}
