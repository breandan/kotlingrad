...
Refactoring is an essential feature in any IDE, and the essence of refactoring is renaming. Consider what must occur when a user wishes to rename a token in her program, such as the \inline{data} parameter on line \#1 below:
%
\begin{pythonlisting}
def callback(data):
    rospy.loginfo(rospy.get_caller_id() + "received data: %s", data.data)
...
\end{pythonlisting}
%
There were four occurrences of the string \inline{data}, only two of which were correctly renamed. Instead, the operation should only touch those identifiers which refer to the variable:
%
...
%
\begin{kotlinlisting}
fun <C, D> unitTest(subprogram: (C) -> D) {
    val input = C(...)
    val expectedOutput = D(...)
    val actualOutput = subprogram(input)
    assertEquals(expectedOutput, actualOutput)
}
\end{kotlinlisting}
%
When carefully applied, it can be quite effective at detecting bugs and documenting the author's belief of preconditions and postconditions. The trouble is, someone needs to write a bunch of test cases for it to work properly. Additionally, it only tests subprograms, and must be updated when the program changes. This has the unintended side effect of decreasing agility, discourage refactoring, or discarding prior work when tests become obsolete.

\subsection{Integration Testing}

\noindent In integration testing, we are not so concerned about the specific behavior of individual subprograms, but the overall behavior of an entire program. Integration tests are often written in the following style:

\begin{kotlinlisting}
fun <T, Y> integrationTest(program: (T) -> Y, inputs: Set<T>>) =
    inputs.forEach { input ->
        try {
            assert(checkPostCondition(program(input)))
...
\end{kotlinlisting}
%
With this strategy, there are fewer tests to write as we only care about end-to-end behavior. Integration is an effective smoke test, to ensure that the program can run until termination, and some post condition holds.

For simplicity, in the following sections, we will only consider examples of tests and functions which are pure functions, i.e. which have no side effect.

\subsection{Oracle Testing}

Oracle testing is an automated software testing methodology which generates random inputs to test a given program. For example, consider the following test:
%
\begin{kotlinlisting}
fun <reified T, Y> simpleFuzzTest(program: (T) -> Y, oracle: (T) -> Y, budget: Int = 100) {
    if(budget == 0) return
    val input: T = generateRandomInput(T::class)
    assertEquals(program(i), oracle(i))
    simpleFuzzTest(program, oracle, budget--)
}
\end{kotlinlisting}
%
...
%
\begin{kotlinlisting}
tailrec fun <reified T, Y> propertyTest(program: (T) -> Y, hasProperty: (Y) -> Boolean) {
    val input = generateRandomInput(T::class)
    val output = program(input)
    if(!hasProperty(output)) {
        val shrunken: Y = shrink(input, program, property)
        assert("Minimal counterexample of $property: $shrunken")
    }
    propertyTest(program, hasProperty)
}
\end{kotlinlisting}
%
...
\end{kotlinlisting}
%
The trouble is, finding the right properties to test can be highly sensitive, and requires a lot of effort and domain-specific expertise. In addition, the user needs to specify a custom shrinker for performance, which is not always clear how to implement efficiently. What if there were a better way?

\subsection{Metamorphic testing}\label{subsec:metamorphic-testing}

Metamorphic testing is a property testing methodology which addresses the test oracle problem, and specifically the challenge of designing low-cost, general-purpose properties. It has been successfully applied in testing driverless cars~\citep{zhou2019metamorphic, pei2017deepxplore, tian2018deeptest} and other stateful deep learning systems~\citep{du2018deepcruiser}.
...
%
\begin{kotlinlisting}
fun <Q, S> metamorphicTest(program: (Q) -> S, mr: (Q, S, Q, S) -> Boolean) {
    while(true) {
        val (input: Q, output: ) = drawSampleFromData()
        val tx: (Q) -> Q = genLegalTx(program, mr, input, output)
        val txInput: Q = tx(input)
        val txOutput: S = program(txIn)
...
%
\begin{kotlinlisting}
fun <Q, S> genLegalTx(program: (Q) -> S, mr: (Q, S, Q, S) -> Boolean, input: Q, output: S) {
    while(true) {
        val tx: (Q) -> Q = genRandomTx() // Samples a random transformation
        val txInput: Q = tx(input)
        val txOutput: S = program(txIn)
