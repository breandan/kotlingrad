...
Mom, Gimmey, Mark, Dad, Hanneli Tavante for teaching me type theory and the beauty of functional programming, Siyan Wang for his friendship and adventures, Xiaoyan Liu for planting in me the seed of mathematics, Uncle Andy for watering the seed, Aunt Shannon, and Adam Devoe, Jacquie Kirrane for encouraging me to pursue grad school, Arthur Nunes-Harwitt for teaching me how to think like a computer scientist, Renee Miller for sparking my interest in neural science, Ian Clarke for teaching me clean code, Kotlin and multi-armed bandits, Hadi Hariri for putting more faith than in me than I deserved, Eugene Petrenko for his advice about grad school, Rusi Hristov for teaching me Docker and bash. Isabella Albuquerque and Jo\~ao Monteiro for teaching me the meaning of hard work and how to write a good paper. Manfred Diaz and Maxime Chevalier Boisvert for the inspiration, conversations and feedback. Florian Golemo for setting the bar of excellence in research. Ryan Turner, Saikrishna Gottipati and Vincent Mai for the interesting conversations and adventures. Olivier Breleux for teaching me Myia, Bart van Merri\"enboer for showing me a path between ML and PL, Dmitry Serdyuk for introducing me to Montr\'eal, Christian Perone for the introduction to Pytorch, Kiran Gopinathan for providing useful comments and feedback, Miltos Allamanis for showing me there is a place for PL/SE in ML, Celine Begin at the Universit\'e de Montr\'eal for giving a strange visitor a chance on a cold winter's day, Stefan Monnier for thoughtfully and thoroughly replying to my long-winded emails, Andrea Censi for his encouragement, (last but not least) my wonderful advisors Liam Paull for trust, encouragement, and giving me far more credit than I deserved, and Michalis Famelis for teaching me about intuistionistic logic, formal methods, and self-discipline.
...
\begin{figure}
    ...
    \caption{Projectional editors such as MPS~\citep{voelter2010language, pech2013jetbrains} (shown above)  are able to render plaintext source code in alternate formats for reading and manipulation.}
...
\begin{algorithmic}[1]
\Procedure{BfAdversary}{$\mathbf{\hat P}: \mathbb{R}^m \rightarrow \mathbb{R}, g_0: \mathbb R$, $g_1: \mathbb R$, $\ldots$, $g_{m-1}: \mathbb R$}
...
\State \Return $\argmax{}\{\mathbf{\hat P}(g_0 + c) \circ$\Call{BfAdversary}{$\mathbf{\hat P}(g_0 + c), g_1, \ldots, g_{m-1}$},\newline
\hspace*{10.5em} $\mathbf{\hat P}(g_0 - c)\circ$\Call{BfAdversary}{$\mathbf{\hat P}(g_0 - c), g_1, \ldots, g_{m-1}$},\newline
\hspace*{10.5em} $\mathbf{\hat P}(g_0)\circ$\Call{BfAdversary}{$\mathbf{\hat P}(g_0), g_1, \ldots, g_{m-1}$}$\}$
\EndIf
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\If {$m = 1$} \Comment{Compute finite difference and perform gradient ascent (G.A.)}
\State \Return $g_0 + \frac{\mathbf{\hat P}(g_0) - \mathbf{\hat P}(g_0 + c)}{c}$
\Else \Comment{Apply single-step G.A. using componentwise finite difference and recurse}
...
\end{equation}
%
If the adversary has zero knowledge about the program's implementation or the data distribution, $D_{\mathbf{\hat P}}$, she can do no better than random search~\citep{wolpert1997no}. However, if she has some information about the input distribution, she could re-parameterize that distribution to incorporate her knowledge. Assuming the program has previously been tested on common inputs, she might use the inverse distribution $x \sim \frac{1}{D_{\mathbf{\hat P}}}$ to select inputs that are less frequent in hopes of finding one not previously considered. In addition, if the tester knew some details of how $\mathbf{\hat P}$ were implemented, she could use classical fuzzing techniques to prioritize the search inputs more likely to break $\mathbf T$. But these methods are not specialized to continuous differentiable functions.
...
\begin{figure}
    ...
    \caption{Browser interface for individual Duckiebots. It is provided by \href{https://www.portainer.io/}{Portainer}, a RESTful web dashboard, which wraps the Docker CLI and offers support for container management, configuration, networking and terminal emulation (seen above), provided by xterm.js which is accessible at: \menu{{\url{http://DUCKIEBOT_NAME:9000/#/container/container_name}} > ``Console''}}
