...
\newtcblisting{xmllisting}[1][]{%
    ...
    underlay unbroken and first={%
        \path[draw=none] (interior.north west) rectangle node[white]{\includegraphics[width=5mm]{file.png}} ([xshift=-10mm,yshift=-9mm]interior.north west);
    }
}
...
\begin{xmllisting}[title=./catkin\_ws/src/pubsub/pubsub.launch]
<launch>
...
To build and run the application, the following commands are required:
...
Rather than interacting with the command line, it would be convenient if there were a graphical tool which could perform all these tasks. Additionally, if there were a navigable reference or typographical error in the launch file, it would be helpful to flag it:
...
Note, how the typographical error is printed in red and the file reference is underlined in blue, indicating it can be selected. These are the kinds of UI features that IDEs provide.
...
\begin{figure}
    ...
    \caption{Hatchery's graphical user interface.}
...
Hatchery can parse URDF, package and launch XML, and \inline{.srv} files.
...
Metamorphic testing is a property-based testing methodology, which relies metamorphic relations. It has been used to test driverless cars~\citep{zhou2019metamorphic, pei2017deepxplore, tian2018deeptest}.
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \While{$j \le$ budget}\Comment{Iterate until count exceeds our budget}
            ...
            \If {$\mathbf T\big(\mathbf{x}_j, \mathcal L(\mathbf{x}_j)\big)$} \Comment{Inside feasible set, gradient ascent}
                \State $D_\mathbf T \gets D_\mathbf T $ + \Call{DiffShrink}{$-\mathcal L, \mathbf{x}_j, \mathbf T$}
            \Else \Comment{Outside feasible set, gradient descent}
                \State $D_\mathbf T \gets D_\mathbf T $ + \Call{DiffShrink}{$\mathcal L, \mathbf{x}_j, \mathbf T$}
            \EndIf
            \State $j \gets j + 1$
        \EndWhile
