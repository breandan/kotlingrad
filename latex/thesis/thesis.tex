...
\noindent With these tools, we can compute the direction in which to update the inputs of a computable function, in order to maximally change the function's output.

For many computable functions, every tiny change to certain inputs produce no change in the function's output. We say that such functions have zero gradient for that input.
...
It is an open question whether non-differentiable functions exist in the real world\cite{buniy2005hilbert}. At the current physical and temporal scale of modern computing (approximately 10nm and 10ns), they do not, but most modern computers are not equipped with the capability to accurately report the true gradient of their approximately discrete function, so for all intents and purposes, many programs implemented by physical computers are non-differentiable, very much by design. Nevertheless, modern computers are capable of approximating continuous functions on $\bathbb{R}^n$ to arbitrary precision given enough time. For most intents and purposes, a fixed precision approximation is sufficient.

There exists a theorem at the heart of machine learning which states that a simple class of functions, composed of a sum, non-linear activation, and linear function can approximate any computable function to arbitrary precision. More precisely, the universal approximation theorem states:
...
However this theorem does not place an upper limit on N. For empirical reasons which are not yet fully clear, one can approximate most naturally-arising statistical functions precisely, in a tractable amount of time by composing these simple functions and updating $w$ using gradient descent.
