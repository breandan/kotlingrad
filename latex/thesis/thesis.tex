...
Most programs are written in procedural languages, due in part to the prevalence of the Turing Machine and Von Neumann architecture. Lambda calculus provides an equivalent language for computing, and one which is more amenable to the automatic calculation of derivatives. In pure procedural programming, the sole purpose of a function is to pass it values, but we have no way of referring to the function itself. Furthermore, variables are frequently overwritten, making it more difficult to calculate their derivatives.

For example, consider the vector function $f(l_1, l_2) = l_1 \cdot l_2$, seen in\ref{tab:1}.
...
Prior work has explored the Dockerization of ROS containers\cite{white2017ros-docker}. This work forms the basis for our workflow, which is extended for Duckietown in particular.

\chapter{Case study: application for autonomous robotics}

As a case implement a case study in building a mobile application using ROS, Docker, Android and sHere, we implement a mobile application using the above toolchain.

\section{Design}

Designed with Hatchery. This tool allows us to
