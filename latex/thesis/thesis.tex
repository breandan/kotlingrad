...
\end{bnflisting}
%
Grammar-Kit consumes this file, and generates Java source code for parsing ROS \inline{.msg} files. Generated sources can be manually edited to provide support for more advanced functionality such as more effective error-recovery, but for parsing relatively simple key-value configuration languages, such as those used by ROS \inline{.msg} and \inline{.srv} files, the PEG BNF grammar presented above is sufficient. Hatchery is also capable of parsing \href{http://wiki.ros.org/urdf}{URDF}, package and launch XML files.
...
\end{pclisting}
%
Hatchery provides assistance for configuring, building and running ROS applications inside a custom graphical user interface (GUI). This GUI effectively serves as a wrapper for the ROS command line interface (CLI). Visual elements like configuration options and flags toggle an internal model called the ``Run/Debug Configuration''. When the user manually triggers the run command, Hatchery's internal model is converted to a string representing the command to be executed and sent to a terminal emulator, which invokes the command and displays the corresponding output.
...
\begin{multicols}{2}
\begin{todolist}
    \item[\done] ROS Launch (\inline{*.launch}, \inline{*.test})
        \item[\done] Syntax highlighting
        \item[\done] Resource references (\inline{\$(find <directory>)...})
    \item[\done] ROS Package (\inline{package.xml})
        \item[\done] Syntax highlighting
        \item[\done] Package references (\inline{<build\_depend>}, \inline{<test\_depend>}, \inline{<run\_depend>})
    \item[\done] ROS URDF (\inline{*.urdf.xacro})
        \item[\done] Syntax highlighting
        \item[\done] Resource references (\inline{\$(find <directory>)...})
    \item[\done] ROS Bag (\inline{*.bag})
    \item Live logfile tracking
    \item[\done] ROS Message (\inline{*.msg})
    \item[\done] Syntax highlighting
    \item[\done] ROS Service (\inline{*.srv})

    %It also supports refactoring and navigation in the following file types through third party plugins:
    %
    %* XML
    %* Python
    %* YAML
    ...
    \item Integration with the Robot Operating System (ROS)
    ...
    \item[\done] Gazebo simulator integration
    \item C/C++ support with build automation
    \item Remote debugging and testing support
    \item Docker integration
    \item[\done] Basic Docker support
    \item Remote host and script support
    \item Docker hub namespace awareness
    \item Support for platformio tooling
    \item X11 forwarding and rqt support
    \item Static analysis
    \item[\done] Invalid dependency detection
    \item Validate Python/msg/srv compatibility
    \item ROS nodes and graph analysis via \inline{rosdep}/\inline{rqt\_dep}
    ...
    \item[\done] \href{http://wiki.ros.org/rqt_dep}{\inline{rqt\_dep}} - Visualize dependecies
    ...
    \item[\done] Detecting and managing ROS installations.
\end{todolist}
\end{multicols}
...
Many more examples of symbolic differentiation exist in various forms. This pattern is unlikely to be mere coincidence, and suggests an unrealized connection between differential and algebraic geometry, perhaps holding important insights for representation learning and differentiable programming.
...
From a performance perspective, it would be advantageous to integrate with a dedicated linear algebra backend such as ND4J~\citep{team2016nd4j}, Apache Commons Math~\citep{developers2012apache}, EJML~\citep{abeles2010efficient} or JBlas~\citep{braun2011jblas}.
