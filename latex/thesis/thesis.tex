...
\section{Designing intelligent robotic systems}
...
\section{Implementing Type Safe Differentiable Programming}
...
\section{Validation and Testing of Differentiable Programs}
...
\section{Software Maintenance and Reproducible Computing}
...
First-class dependent types are needed for arbitrary shape safety (e.g.\ for concatenation or convolution~\citep{xi1998eliminating}), but unnecessary for equality checking, such as when multiplying matrices.\footnote{Less expressive type systems are still capable of performing arbitrary computation in the type checker. As specified, Java's type system is known to be Turing Complete~\citep{Grigore:2017:JGT:3009837.3009871}. It may be possible to emulate a limited form of dependent types in Java by exploiting this property, although this may not be computationally tractable due to the practical limitations noted by Grigore.} Checking ordinary matrix arithmetic is decidable in any type system loosely based on System $\text{F}_{<:}$~\citep{cardelli1991extension}. In practice, we can implement a shape-safe tensor algebra in any language with subtyping and parametric polymorphism, such as \href{https://docs.oracle.com/javase/tutorial/java/generics/index.html}{Java}~\citep{naftalin2007java}, \href{https://kotlinlang.org/docs/reference/generics.html}{Kotlin}~\citep{tate2013mixed} or \href{https://doc.rust-lang.org/1.7.0/book/generics.html}{Rust}~\citep{crozet2019nalgebra}.
...
\chapter{Validation and Testing of Differentiable Programs}\label{ch:difftest}
...
In this chapter we have visited some interesting new ideas for validating intelligent systems, from the perspective of software engineering and machine learning.  We have seen a curious resemblance between some new and old ideas in adversarial learning and automated testing, including property-based testing, metamorphic testing, and generative adversarial networks. We have proposed a framework for evaluating differentiable programs in a low-cost, data-driven manner and suggest some directions for future research in the evaluation of differentiable programs. As today's engineers begin to incorporate learning systems in tomorrow's robotic infrastructure, we believe the increased assurance these tools provide will be an important factor in the end-to-end validation of these highly-interconnected cyberphysical systems.

\chapter{Software Maintenance and Reproducible Computing}\label{ch:ducker}
...
\mediumrare{\chapter{Case study: an application for autonomous robotics}\label{ch:case-study}}
...
\chapter{Software Maintenance and Reproducible Computing}
