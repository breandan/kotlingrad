...
%
\begin{centering}
\menu{File > Settings > Plugins > Marketplace > {\faSearch ``Hatchery''}}
\end{centering}
...
Grammar-Kit consumes this file, and generates source code for parsing ROS \inline{.msg} files. Hatchery is also capable of parsing URDF, package and launch XML, and \inline{.msg}/\inline{.srv} files.
...
\begin{figure}
    ...
    \caption{\inline{roslaunch} configuration. Accessible via: \menu{Run > Edit Configurations > + > ROS Launch}}
...
\end{equation}
%
Likewise, for a scalar function $Q(q_0, q_1, \dots, q_n):  \mathbb{R}^m\rightarrow\mathbb{R}$, the gradient $\nabla Q$ tells us:
%
\begin{equation}
...
\mathbf J = \begin{bmatrix}
                       \dfrac{\partial \mathbf{f}}{\partial x_1} & \cdots & \dfrac{\partial \mathbf{f}}{\partial x_m} \end{bmatrix}
= \begin{bmatrix}
...
By using functional programming, Kotlin$\nabla$ uses the same abstraction to represent mathematical functions and programming functions. All functions in Kotlin$\nabla$ are pure functions, composed of expressions which together form a data-flow graph (DFG). An expression is simply a \inline{Function}, which is only evaluated when invoked with numerical values, e.g. \inline{z(0, 0)}. In this way, Kotlin$\nabla$ is similar to other compiled graph-based frameworks like TensorFlow and Theano.
...
\end{tikzpicture}
\caption{Implicit DAG constructed by the original expression, \inline{z}, seen above.}
    \label{lst:edsl}
\end{figure}
...
But this would be very inefficient. Instead, we should have a more principled way to mutate a given input in our dataset so as to discover invalid outputs.

\subsection{Adversarial Testing}

This leads us to adversarial testing. In the most general case, we are given an input-output pair from an oracle and a program claiming to approximate that oracle, but not necessarily the oracle itself. Our goal is to find a small change to the input value, which when fed to our program, produces the largest change to its output. Imagine a function $\mathbf{\hat P}: \mathbb R^m \rightarrow \mathbb R $, where we seek to change each of the components $x_1, ..., x_m$ by a fixed amount $c \in \mathbb R$, so as to produce the largest output value directly. Suppose that for each input parameter $g_0, g_1, \ldots, g_{m-1}$, we have one of three choices to make: either we can increase the value by $c$, decrease the value by $c$, or leave it unchanged. We are given no further information about $\mathbf{\hat P}$. Consider the na\"ive solution, in which we try every combination of variable perturbations and select the inputs corresponding to the greatest output value:

\begin{algorithm}[H]
\caption{Na\"ive Fuzzer}
\label{alg:naive_fuzz}
\begin{algorithmic}[1]
\Procedure{Na\"iveAdversary}{$\mathbf{\hat P}: \mathbb{R}^m \rightarrow \mathbb{R}, g_0: \mathbb R$, $g_1: \mathbb R$, $\ldots$, $g_{m-1}: \mathbb R$}
\If {$m = 1$}
\State \Return $\argmax{}\{\mathbf{\hat P}(g_0 + c), \mathbf{\hat P}(g_0 - c), \mathbf{\hat P}(g_0)\}$
\Else
\State \Return $\argmax{}\{\mathbf{\hat P}(g_0 + c) \circ$\Call{Na\"iveAdversary}{$\mathbf{\hat P}(g_0 + c), g_1, \ldots, g_{m-1}$},\newline
\hspace*{10.5em} $\mathbf{\hat P}(g_0 - c)\circ$\Call{Na\"iveAdversary}{$\mathbf{\hat P}(g_0 - c), g_1, \ldots, g_{m-1}$},\newline
\hspace*{10.5em} $\mathbf{\hat P}(g_0)\circ$\Call{Na\"iveAdversary}{$\mathbf{\hat P}(g_0), g_1, \ldots, g_{m-1}$}$\}$
\EndIf
\EndProcedure
\end{algorithmic}
\end{algorithm}

As we can see, algorithm \autoref{alg:naive_fuzz} is $\mathcal{O}(3^m)$ with respect to $\mathbf{\hat P}$ - not a very efficient search routine, especially if we have a larger set of perturbances or need to repeat the procedure multiple times. Clearly, if we want to find the best direction in which to update $\mathbf g$, we need to be more careful about how we perform the search.

Assume that $\mathbf{\hat P}$ is differentiable almost everywhere. Consider algorithm \autoref{alg:fd_fuzz} a refinement of algorithm \autoref{alg:naive_fuzz} which uses a finite difference method to approximate the program's derivative with respect to each component of the input. This tells us how to change the input by a incremental amount to produce the largest output, without needing to check every possible perturbation.

\begin{algorithm}[H]
\caption{Finite Difference Adversary}
\label{alg:fd_fuzz}
\begin{algorithmic}[1]
\Procedure{FdAdversary}{$\mathbf{\hat P}: \mathbb{R}^m \rightarrow \mathbb{R}, g_0: \mathbb R$, $g_1: \mathbb R$, $\ldots$, $g_{m-1}: \mathbb R$}
\If {$m = 1$}
\State \Return $g_0 + \frac{\mathbf{\hat P}(g_0) + \mathbf{\hat P}(g_0 + c)}{c}$
\Else
\State \Return $g_0 + \frac{\mathbf{\hat P}(g_0, 0, \ldots) + \mathbf{\hat P}(g_0 + c, 0, \ldots)}{c}$, \Call{FdAdversary}{$\mathbf{\hat P}, g_1, \ldots, g_{m-1}$}
\EndIf
\EndProcedure
\end{algorithmic}
\end{algorithm}

We now have a procedure that is $\mathcal{O}(m)$ with respect to $\mathbf{\hat P}$, but must be recomputed for each input -- we can still do better. Furthermore, we have not yet incorporated any form of gradient descent or constraint on the input values. Perhaps we can combine the notion of metamorphic testing seen in \autoref{subsec:metamorphic-testing} with constrained optimization to accelerate the search for adversarial examples.

During backpropogation we perform gradient descent on a differentiable function with respect to its parameters for a specific set of inputs. In gradient-based adversarial testing, we perform gradient descent on a loss function with respect to the inputs using a fixed parameter setting. Suppose we have a differentiable vector function $\mathbf{\hat P}: \mathbb{R}^m\rightarrow\mathbb{R}^n$, defined as follows:
%
...
\end{equation}
%
Recall we can solve for the gradient using \autoref{eq:vfun_chain_rule} by taking the product of the Jacobians, $\mathbf{J}_{\mathbf{p}_0} \ldots \mathbf{J}_{\mathbf{p}_k}$ with respect to $\mathbf{\Theta}$. In white box adversarial learning, we are given a fixed $\mathbf \Theta$\footnote{In contrast with backpropogation, where the parameters $\mathbf\Theta$ are updated.} and control the value of $\mathbf x$, so we can rewrite $\mathbf{\hat P}_k(\mathbf{x}^{(i)};\mathbf\Theta)$ instead as $\mathbf{\hat P}(\mathbf x)$, and take the gradient directly with respect to $\mathbf x$. Concretely, our objective is to find the ``worst'' $\mathbf x$ within a small distance of any $\mathbf x^{(i)}$, i.e. where $\mathbf{P}(\mathbf x)$ least resembles $\mathbf{\hat P}(\mathbf x)$:
%
\begin{equation}
\mathbf{x}^* = \argmax{\mathbf{x}}\mathcal{L}\big(\mathbf{\hat P}(\mathbf{x}), \mathbf{y}^{(i)}\big) \text{ subject to } CS = \{\mathbf{x} \in \mathbb{R}^m \text{ s.t. } ||\mathbf{x}^{(i)} - \mathbf{x}|| < \epsilon\}
\end{equation}
%
 To do so, we initialize $\mathbf{x} \sim U[CS]$ and perform projected gradient descent on the loss:
%
...
\begin{algorithm}[H]
    ...
    \label{alg:prob_fuzz}
    \begin{algorithmic}[1]
        \Procedure{ProbFuzz}{$\mathcal L: \mathbb R^m \rightarrow \mathbb R$, $\mathcal S_n$, $\mathbf T: \mathbb R^m \times \mathbb R \rightarrow \mathbb B$, budget: $\mathbb Z^+$}
...
\begin{algorithm}[H]
    ...
    \label{alg:gd_shrink}
    \begin{algorithmic}[1]
        \Procedure{DiffShrink}{$\mathcal L: \mathbb R^m \rightarrow \mathbb R$, $\mathbf{x}_0: \mathbb R^m$, $\mathbf T: \mathbb R^m \times \mathbb R \rightarrow \mathbb B$}
        \State $i \gets 1$
...
%
In addition,
We assume the following software dependencies have been installed on (3):
%
...
Our largest single contribution to the Duckietown project involved writing a script for flashing bootable media with a DuckieOS image. Prior to its creation, installation was a manual and time-consuming process. Now, the following command is all that is needed:
...
To access the DuckieOS web interface, visit following the URL in the web browser: \url{http://DUCKIEBOT_NAME:9000/}. If the installation successfully completed and the network is configured properly, the page displayed in~\autoref{fig:portainer_ui} should be accessible.

\begin{figure}
    \includegraphics[width=0.80\textwidth]{portainer_screenshot.png}
    \caption{Browser interface for individual Duckiebots. It is provided by \href{https://www.portainer.io/}{Portainer}, a wrapper for the Docker CLI with support for container management and terminal emulation, which is accessible at: \menu{{\url{http://DUCKIEBOT_NAME:9000/#/container/container_name}} > ``Console''}}
