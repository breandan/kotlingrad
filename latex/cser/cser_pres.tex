...
\begin{document}
    ...
    \begin{frame}
        ...
        \end{align*}
        %
        Where $\boldsymbol\Theta \in \mathbb{R}^{\text{max}(n, m) \times k}$ are free parameters and $\mathbf{x} \in \mathbb{R}^n$ is a single input. Assume we are given $\mathbf{Y} = [\mathbf{P}(\mathbf{x}^{(1)}), \dots, \mathbf{P}(\mathbf{x}^{(z)})]$ from an oracle. In order to approximate $\mathbf{Y}$, repeat the following procedure until $\boldsymbol\Theta$ converges:
        %
        \begin{align*}
